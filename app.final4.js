...
const elAlertModeFixed=$('alertModeFixed'), elAlertModeAtr=$('alertModeAtr'), elAlertFixedVal=$('alertFixedVal'), elAlertAtrPct=$('alertAtrPct');
const elLiveAgeOkSec=$('liveAgeOkSec');
...
function checkProximityAlert(entry){
  ...
  const dynThr = Number.isFinite(atrNow) ? Math.max(thr, 0.25 * atrNow) : thr;
...
async function refreshLive(){
  try{
    Promise.resolve(fetchLivePrice()).then(function(price){
      var __now = Date.now();
      if (typeof price === 'number' && isFinite(price)) {
        var prev = (typeof window.__livePrice === 'number' && isFinite(window.__livePrice)) ? window.__livePrice : null;
        if (prev != null) {
          var pct = Math.abs(price - prev) / Math.max(1e-9, Math.abs(prev));
          if (pct > 0.007) { try{ console.warn('Spike filtered', pct); }catch(_){} return; }
        }
        try { if (typeof paintLive === 'function') { paintLive(price, __now); } } catch(_) {}
        window.__livePrice = price;
        window.__liveTimeMs = __now;
        try { if (typeof reprojectWithLive === 'function') { reprojectWithLive(); } } catch(_) {}
      }
    }).catch(function(e){ try{ console.warn('Live error:', e); }catch(_){} });
  } catch(e){ try{ console.warn('Live error:', e); }catch(_){} }
}
...
try {
  var s = window.__seriesForExport;
  if (!Array.isArray(s) || !s.length) {
    // try alternative known globals
    if (Array.isArray(window.__lastSeries) && window.__lastSeries.length) {
      s = window.__lastSeries.slice();
    }
  }
  if (!Array.isArray(s) || !s.length) { 
  ...
  s = (typeof mergeWithLive === 'function') ? mergeWithLive(s) : s;
  var DELIM = ';', CRLF = '\r\n', BOM = '\ufeff';
  var q = function(v){ return '"' + String(v).replace(/"/g,'""') + '"'; };
  var header = ['Date','Open','High','Low','Close'];
  var rows = s.map(function(r){
    var iso = new Date(r.ts).toISOString().replace('T',' ').slice(0,19);
    return [iso, r.open, r.high, r.low, r.close];
  });
  var csv = [header.map(q).join(DELIM)].concat(rows.map(function(row){ return row.map(q).join(DELIM); })).join(CRLF) + CRLF;
  var blob = new Blob([BOM + csv], {type:'application/vnd.ms-excel;charset=utf-8'});
  var nameStamp = (typeof __stamp === 'string' && __stamp) ? __stamp : (function(){ 
    var d=new Date(); 
    function pad(n){ return (n<10?'0':'')+n; } 
    return d.getFullYear()+'-'+pad(d.getMonth()+1)+'-'+pad(d.getDate())+'_'+pad(d.getHours())+'-'+pad(d.getMinutes()); 
  })();
  var tf = (window.currentTF || 5);
  var fname = 'XAUUSD_' + tf + 'min_merged_' + nameStamp + '.csv';
...
// Some older pages call wirePreExportBtn(); keep it mapped to the new function.
try { if (!window.wirePreExportBtn) { window.wirePreExportBtn = function(){ try{ wireExportBtn(); }catch(_){ } }; } } catch(_){}
})();
...
/* =====================
   GoldSignals App Patch
   - Keep source priority: WORKER then GOLD-TICKS
   - Freshness guard, no-cache fetch, immediate chart sync
   - Auto-telegram from #summaryText (if present)
   Notes: Add-only patch; does not remove existing code.
===================== */
(function(){
  // ----- Helper: worker-first live fetch with freshness guard -----
  if (!window.gsFetchLiveWorkerFirst){
    ...
    const N = n => (Number.isFinite(+n) ? +n : null);

    async function getLiveFromWorkerFirst(){
      const now = Date.now();
      let w=null;
    ...
    async function applyLive(t){
      if (!t) return;
      const p = N(t.price);
      const ts = N(t.ts) ?? Date.now();
      if (p==null) return;
      window.__livePrice  = p;
      window.__liveTimeMs = ts;
      window.LAST_LIVE    = { price: p, timeMs: ts };
      if (typeof window.reprojectWithLive === "function"){
        try{ window.reprojectWithLive(); }catch(_){}
      }
      if (typeof window.updateLiveLabel === "function"){
        try{ window.updateLiveLabel(p); }catch(_){}
      }
    }

    window.gsFetchLiveWorkerFirst = async function(){
      const t = await getLiveFromWorkerFirst();
      await applyLive(t);
      return t;
    };
  }

  // ----- Singleton poller (non-invasive). Starts only if not already running -----
  if (!window.__GS_POLL_LIVE__){
    window.__GS_POLL_LIVE__ = true;
    (async function loop(){
      while(true){
        try{ await window.gsFetchLiveWorkerFirst(); }catch(_){}
        await new Promise(r=>setTimeout(r, 1000));
      }
    })();
  }

  // ----- Auto Telegram from #summaryText (optional) -----
  (function(){
    const WORKER_ALERT = "https://workerjs.samer-mourtada.workers.dev/alert";
    function parseSide(t){ if(/شراء/.test(t))return"BUY"; if(/بيع/.test(t))return"SELL"; return null; }
    function parseEntry(t){ const m=/سعر الدخول:\s*([\d.]+)/.exec(t||""); return m?parseFloat(m[1]):null; }
    function tfShort(){
      const tf = (typeof window.currentTF!=='undefined'?window.currentTF:(window.__cache&&window.__cache.tf)||5);
      return tf===1440?'1d':(tf+'m');
    }
    let lastId=null;
    async function maybeSend(text){
      text = (text||'').trim();
      if (!text || /لا توجد نصيحة|مرفوض|حيادي/.test(text)) return;
      const side = parseSide(text); if (!side) return;
      const entry = parseEntry(text) ?? (typeof window.__livePrice==='number'?window.__livePrice:null);
      const id = side+'|'+tfShort()+'|'+(entry??'na');
      if (lastId===id) return;
      lastId = id;
      let tp1=null,tp2=null,sl=null;
      try{
        if (typeof window.gsComputeTargets==='function'){
          const t = window.gsComputeTargets(entry, side);
          tp1=t.tp1; tp2=t.tp2; sl=t.sl;
        }
      }catch(_){}
      try{
        await fetch(WORKER_ALERT,{
          method:'POST',
          headers:{'content-type':'application/json'},
          body: JSON.stringify({ side, tf: tfShort(), entry, tp1, tp2, sl, filtersRejected:false })
        });
      }catch(_){}
    }
    ...
    function wire(){
      ...
      if (!el || typeof MutationObserver==='undefined') return false;
      const mo = new MutationObserver(()=> maybeSend(el.textContent));
...
/* ===== Live Overlay Reliability Booster (add-only) =====
   Fixes cases where live line disappears after reload or toggling "دقيق".
   Strategy:
   - Keep LAST_LIVE sticky and re-apply projection on a lightweight cadence.
   - Re-apply on common UI/DOM events that rebuild the chart (checkbox changes, CSV reloads).
   - No changes to existing functions; safe to drop in.
*/
(function(){
  try{
    // Call reprojectWithLive periodically (idempotent & cheap in our app)
    if (!window.__GS_FORCE_LIVE_LOOP__){
      window.__GS_FORCE_LIVE_LOOP__ = true;
      (function loop(){
        try{
    ...
    // Re-apply on visibility/focus (after reloads or tab switches)
    ['visibilitychange','focus','resize','hashchange','pageshow','load'].forEach(ev=>{
      addEventListener(ev, ()=>{
        try{
          if (window.LAST_LIVE && typeof window.reprojectWithLive==='function'){
    ...
    // Re-apply on any checkbox/select change (e.g., "Precise" / الإطار الزمني)
    ...
    document.addEventListener('click', (e)=>{
      ...
      if (/CSV|حساب الإشارات الآن|تحديث تلقائي/i.test(text)){
        try{
          setTimeout(()=>{
            if (window.LAST_LIVE && typeof window.reprojectWithLive==='function'){
              window.reprojectWithLive();
...
/* ===== Unify Live Source (override fetchLivePrice) =====
   Ensures KPI and chart use the SAME number.
   - If __livePrice was updated in the last 1200ms, reuse it (no second fetch).
   - Otherwise, get from gsFetchLiveWorkerFirst() and return price.
   Keeps original function available via window.__origFetchLivePrice if ever needed.
*/
(function(){
  try{
    if (typeof fetchLivePrice === 'function' && !window.__fetchLiveUnified__){
      window.__fetchLiveUnified__ = true;
      window.__origFetchLivePrice = fetchLivePrice;
      fetchLivePrice = async function(){
        try{
          const now = Date.now();
          ...
          if (typeof window.gsFetchLiveWorkerFirst === 'function'){
            const t = await window.gsFetchLiveWorkerFirst();
            if (t && Number.isFinite(t.price)) return t.price;
          }
        }catch(_){}
        // Fallback to original behavior
        try{ return await window.__origFetchLivePrice(); }catch(e){ throw e; }
      };
    }
  }catch(_){}
})();
...
/* ===== Live Unifier v2: hook paintLive =====
   Guarantees the chart overlay uses EXACTLY the same price shown in the KPI.
   - Wraps paintLive to also set LAST_LIVE and trigger reproject.
   - Add-only, safe.
*/
(function(){
  try{
    if (typeof window.paintLive === 'function' && !window.__paintLiveHooked__){
      window.__paintLiveHooked__ = true;
      window.__origPaintLive = window.paintLive;
      window.paintLive = function(price, ts){
        try{
          const _ts = Number.isFinite(ts) ? ts : Date.now();
          if (Number.isFinite(price)){
            window.__livePrice  = price;
            window.__liveTimeMs = _ts;
            window.LAST_LIVE    = { price, timeMs: _ts };
          }
        }catch(_){}
        try{ return window.__origPaintLive.apply(this, arguments); }
        finally{
          try{
            if (typeof window.reprojectWithLive === 'function'){
              // re-apply twice to cover any immediate canvas rebuilds
              setTimeout(window.reprojectWithLive, 0);
              setTimeout(window.reprojectWithLive, 200);
            }
          }catch(_){}
        }
      };
