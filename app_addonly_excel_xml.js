...
/* === ADD-ONLY: Force unique filename + purge old listeners on Export button === */
(function(){
  function __forceUniqueName(prefix){
    const d=new Date();
    const pad=n=>String(n).padStart(2,'0');
    const stamp = `${d.getFullYear()}${pad(d.getMonth()+1)}${pad(d.getDate())}_${pad(d.getHours())}${pad(d.getMinutes())}${pad(d.getSeconds())}`;
    const rand = Math.random().toString(36).slice(2,7);
    return `${prefix}_${stamp}_${rand}.csv`;
  }
  function __replaceBtn(id){
  ...
  function __wrapExporter(btn){
    const preferXlsOnAndroid = /Android/i.test(navigator.userAgent);
    const hasXls = typeof window.downloadMergedXls === 'function';
    const hasCsv = typeof window.downloadMergedCsv === 'function';

    btn.addEventListener('click', function(){
      try{
        if(preferXlsOnAndroid && hasXls){ window.downloadMergedXls(); return; }
        if(!hasCsv){ alert('وظيفة التصدير غير متاحة في هذا الإصدار'); return; }
        const freshName = __forceUniqueName(`XAUUSD_${window.currentTF||5}min_merged`);
        // Intercept next anchor to enforce a new filename
        const _append = document.body.appendChild;
        document.body.appendChild = function(node){
          if(node && node.tagName==='A'){ try{ node.setAttribute('download', freshName); }catch{} }
          return _append.call(this, node);
        };
        try{ window.downloadMergedCsv(); }
        finally{ document.body.appendChild = _append; }
      }catch(e){ console.error(e); alert('فشل تنفيذ التصدير: '+(e.message||e)); }
    });
  }
  function __wire(){
    let btn = __replaceBtn('btnExportCsv');
    if(btn){ __wrapExporter(btn); }
  }
  if(document.readyState==='loading') document.addEventListener('DOMContentLoaded', __wire);
  else __wire();
})();


/* === ADD-ONLY PATCH: Force XLS export (Office Mobile compatible) === */
(function(){
  // Fallback XLS exporter if not present
  if (typeof window.downloadMergedXls !== 'function'){
    window.downloadMergedXls = function(){
      let series = window.__seriesForExport || window.__lastBaseSeries || window.__cache?.series || [];
      if(!series || !series.length){ alert('ما في بيانات للتصدير.'); return; }
      ...
      const rowsHtml = series.map(r=>{
        ...
        return `<tr><td>${esc(date)}</td><td>${esc(r.open)}</td><td>${esc(r.high)}</td><td>${esc(r.low)}</td><td>${esc(r.close)}</td></tr>`;
      }).join('');
      const html = `<!doctype html><html><head><meta charset="utf-8"></head><body><table border="1"><tr><th>Date</th><th>Open</th><th>High</th><th>Low</th><th>Close</th></tr>${rowsHtml}</table></body></html>`;
      const blob = new Blob([html], { type: 'application/vnd.ms-excel;charset=utf-8' });
      const d=new Date(); const pad=(n)=>String(n).padStart(2,'0');
      const fname = `XAUUSD_${(window.__cache?.tf||window.currentTF||5)}min_merged_${d.getFullYear()}${pad(d.getMonth()+1)}${pad(d.getDate())}_${pad(d.getHours())}${pad(d.getMinutes())}${pad(d.getSeconds())}.xls`;
      const a=document.createElement('a'); a.href=URL.createObjectURL(blob); a.download=fname;
      document.body.appendChild(a); a.click(); setTimeout(()=>{URL.revokeObjectURL(a.href); a.remove();},0);
    };
  }

  // Replace export button and wire to XLS only
  function __replaceBtn(id){
    const old=document.getElementById(id);
    if(!old) return null;
    const clone=old.cloneNode(true);
  ...
  function __wire(){
    const btn = __replaceBtn('btnExportCsv');
    if(btn && !btn.__wired){
      btn.addEventListener('click', function(e){
        try{ window.downloadMergedXls(); }catch(err){ alert('فشل تصدير XLS: '+(err.message||err)); console.error(err); }
      });
      btn.__wired=true;
    }
  }
...
/* === ADD-ONLY PATCH: Bind to EXISTING top/header buttons by text (Arabic) & hide floating panel === */
(function(){
  function text(el){ return (el.textContent||'').replace(/\s+/g,' ').trim(); }
  function match(el, re){ try{ return re.test(text(el)); }catch{ return false; } }
  ...
  function wireExisting(){
    ...
    const reImport = /(استيراد\s*CSV|CSV\s*استيراد)/i;
    const reExport = /(تنزيل\s*CSV|CSV\s*تنزيل|تحميل\s*CSV)/i;

    const file = ensureHiddenFileInput();
    if (!file.__wired){
      file.addEventListener('change', async (ev)=>{
        ...
        try{
          ...
          if (typeof window.parseCsv === 'function'){
            const rows5 = window.parseCsv(text);
    ...
    let foundImport=false, foundExport=false;
    for (const el of nodes){
      if (!foundImport && match(el, reImport) && !el.__wiredImport){
        el.addEventListener('click', (e)=>{ e.preventDefault(); file.click(); });
        el.__wiredImport = true; foundImport = true;
      }
      if (!foundExport && match(el, reExport) && !el.__wiredExport){
        el.addEventListener('click', (e)=>{ e.preventDefault(); try{ window.downloadMergedXls(); }catch(err){ alert('فشل التصدير: '+(err.message||err)); } });
        el.__wiredExport = true; foundExport = true;
      }
      if (foundImport && foundExport) break;
    }

    // Hide our floating panel if it exists
    const panel = document.getElementById('gsPanel');
    if (panel) panel.style.display='none';
  }

  if (document.readyState === 'loading') document.addEventListener('DOMContentLoaded', wireExisting);
  else wireExisting();
})();



/* === ADD-ONLY PATCH: Export as Excel 2003 XML (SpreadsheetML) for maximum Office Mobile compatibility === */
(function(){
  function xmlEsc(s){ return String(s).replace(/&/g,'&amp;').replace(/</g,'&lt;').replace(/>/g,'&gt;'); }
  function pad(n){ return String(n).padStart(2,'0'); }
  function buildSpreadsheetML(series){
    const header = ['Date','Open','High','Low','Close'];
    const rows = series.map(r=>{
      const d = new Date(r.ts);
      const date = `${d.getFullYear()}-${pad(d.getMonth()+1)}-${pad(d.getDate())} ${pad(d.getHours())}:${pad(d.getMinutes())}:${pad(d.getSeconds())}`;
      return [date, r.open, r.high, r.low, r.close];
    });
    ...
    // header
    xml += '<Row>' + header.map(h=>`<Cell><Data ss:Type="String">${xmlEsc(h)}</Data></Cell>`).join('') + '</Row>\n';
    // data
    for(const row of rows){
      xml += '<Row>'
      + `<Cell><Data ss:Type="String">${xmlEsc(row[0])}</Data></Cell>`
      + `<Cell><Data ss:Type="Number">${Number(row[1])}</Data></Cell>`
      + `<Cell><Data ss:Type="Number">${Number(row[2])}</Data></Cell>`
      + `<Cell><Data ss:Type="Number">${Number(row[3])}</Data></Cell>`
      + `<Cell><Data ss:Type="Number">${Number(row[4])}</Data></Cell>`
      + '</Row>\n';
    }
    xml += '</Table>\n</Worksheet>\n</Workbook>';
    return xml;
  ...
  // Public exporter
  window.downloadMergedExcelXml = function(){
    let series = window.__seriesForExport || window.__lastBaseSeries || window.__cache?.series || [];
    if(!series || !series.length){ alert('ما في بيانات للتصدير.'); return; }
    const xml = buildSpreadsheetML(series);
    const blob = new Blob([xml], { type: 'application/vnd.ms-excel;charset=utf-8' });
    const d = new Date(); const p=n=>String(n).padStart(2,'0');
    const fname = `XAUUSD_${(window.__cache?.tf||window.currentTF||5)}min_merged_${d.getFullYear()}${p(d.getMonth()+1)}${p(d.getDate())}_${p(d.getHours())}${p(d.getMinutes())}${p(d.getSeconds())}.xls`; // keep .xls extension
    const a = document.createElement('a'); a.href = URL.createObjectURL(blob); a.download = fname;
    document.body.appendChild(a); a.click(); setTimeout(()=>{ URL.revokeObjectURL(a.href); a.remove(); },0);
  };

  // Prefer the new SpreadsheetML exporter on all devices (bind existing button again)
  function rewire(){
    const nodes = Array.from(document.querySelectorAll('button, a, [role="button"]'));
    const reExport = /(تنزيل\s*CSV|CSV\s*تنزيل|تحميل\s*CSV)/i;
    for(const el of nodes){
      if(reExport.test((el.textContent||'').replace(/\s+/g,' ').trim()) && !el.__wiredXml){
        const clone = el.cloneNode(true); el.replaceWith(clone);
        clone.addEventListener('click', function(e){ e.preventDefault(); try{ window.downloadMergedExcelXml(); }catch(err){ alert('فشل تصدير Excel: '+(err.message||err)); } });
        clone.__wiredXml = true;
        break;
      }
    }
  }
