...
// آخر ختمة وصول للسعر الحي (إذا مش موجودة).
window.__liveTimeMs = window.__liveTimeMs || 0;

// 1) حدّث "عمر" اللايف على الواجهة إذا عندك عنصر (اختياري، غير مضر)
...
// 2) ووتشدوغ: إذا تأخّر التحديث أكتر من 15 ثانية، اطلب refreshLive() إجباريًا
  ...
  setInterval(function(){
    try {
      ...
      if (age > THRESH_MS && typeof refreshLive === 'function') {
        ...
        refreshLive(); // ما منلغي التايمر القديم — بس منعطي دفشة إضافية
      }
    } catch(e){}
  }, 5000);
})();

// 3) رجّع اللايف فورًا عند الرجوع للتبويب (Android/Chrome أحيانًا يجمّد المؤقّت)
...
// 4) (اختياري) إبقاء الشاشة يقِظة إن توفر Wake Lock — ما بيكسر شي لو مش مدعوم
