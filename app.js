...
/* === Auto Mode: choose indicators based on market state === */
function applyAutoPreset(metrics){
  // metrics: {atrPct, bbWidthPct}
  const elAuto    = document.getElementById('autoInd');
  const elPrecise = document.getElementById('proMode');
  const elMTF     = document.getElementById('mtfConfirm');
  ...
  const set = (el,val)=>{ if(el){ el.checked = !!val; } };
  ...
  // Force Auto indicators ON in Auto mode
  if (elAuto) elAuto.checked = true;

  // Policy: if active -> fast (2of3), else safe (all)
  if (active){
    PRO_MODE = false; MTF_CONFIRM = false; window.gsEntryPolicy = '2of3';
  } else {
    PRO_MODE = true;  MTF_CONFIRM = true;  window.gsEntryPolicy = 'all';
  }
  if (elPrecise) elPrecise.checked = PRO_MODE;
  if (elMTF)     elMTF.checked     = MTF_CONFIRM;
  ...
  // In Auto, lock manual controls to avoid drift
  if (typeof setManualIndicatorsEnabled==='function') setManualIndicatorsEnabled(false);
}
...
function __initGsModeToggle(){
  ...
  setManualIndicatorsEnabled(GS_MODE==='fast');
...
async function runAnalysis(){
  try{
    ...
    (function(){
      ...
      if (GS_MODE==='auto') { applyAutoPreset({atrPct: atrPctVal, bbWidthPct: bbwPctVal}); updateModeUI(act.mode, act.reason); } else if (act && act.mode) {
