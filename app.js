...
/* ===== Quick Mode: fast / safe / auto =============================== */
const GS_MODE_KEY = 'gs_mode';
let GS_MODE = localStorage.getItem(GS_MODE_KEY) || 'auto'; // 'fast' | 'safe' | 'auto'


function applyFlagsForMode(mode){
  // لا نلمس خيارات NY/Pivot إلا من خلال الـ UI (checkboxes) — فقط نعدّل الدقّة و الـ MTF وسياسة الدخول
  if(mode === 'fast'){
    PRO_MODE = false;           // Precise off
    MTF_CONFIRM = false;        // MTF off
    window.gsEntryPolicy = '2of3';
    // Auto indicators (اختياري): خليه مطفّى بالوضع السريع لو الزر موجود
    if (typeof elAutoInd !== 'undefined' && elAutoInd) elAutoInd.checked = false;
  } else if(mode === 'safe'){
    PRO_MODE = true;            // Precise on
    MTF_CONFIRM = true;         // MTF on
    window.gsEntryPolicy = 'all';
    // فعّل Auto indicators بالوضع الحذر لو الزر موجود
    if (typeof elAutoInd !== 'undefined' && elAutoInd) elAutoInd.checked = true;
  }
}
else if(mode === 'safe'){
    PRO_MODE = true;
    MTF_CONFIRM = true;
    window.gsEntryPolicy = 'all';
    if (typeof elToggleNyHours !== 'undefined' && elToggleNyHours) elToggleNyHours.checked = false;
    if (typeof elTogglePivotFilter !== 'undefined' && elTogglePivotFilter) elTogglePivotFilter.checked = false;
  }
}

function detectMarketActivityForAuto(ind){
  // ind: {atrPct, bb:{upper,lower,mid}}
  let atrp = Number(ind?.atrPct);
  let bbw = NaN;
  if (ind?.bb && Number.isFinite(ind.bb.upper) && Number.isFinite(ind.bb.lower) && Number.isFinite(ind.bb.mid) && ind.bb.mid !== 0){
    bbw = ((ind.bb.upper - ind.bb.lower) / ind.bb.mid) * 100;
  }
  const active = (Number.isFinite(atrp) && atrp >= 0.35) || (Number.isFinite(bbw) && bbw >= 0.90);
  const reasons = [];
  if (Number.isFinite(atrp) && atrp >= 0.35) reasons.push(`ATR% ${atrp.toFixed(2)}≥0.35`);
  if (Number.isFinite(bbw) && bbw >= 0.90) reasons.push(`BBW ${bbw.toFixed(2)}≥0.90`);
  return {active, reason: reasons.join(' / ') || 'quiet'};
}

function computeActiveModeForNow(ind){
...
function updateModeUI(mode, reason){
  ...
  if (note){
    const label = (GS_MODE==='auto') ? `ذكي → فعّال حاليًا: <b>${mode==='fast'?'سريع':'حذر'}</b>${reason?` (<span style="opacity:.8">${reason}</span>)`:''}` : (mode==='fast'?'سريع':'حذر');
    note.innerHTML = `الوضع الحالي: <b>${GS_MODE==='auto'?'ذكي':(mode==='fast'?'سريع':'حذر')}</b>` + (GS_MODE==='auto' ? ` — ${label}` : '');
  }
}
/* ==================================================================== */
...
function quickPolicySide(ctx){
  if(!ctx) return null;
  const trendUp  = Number.isFinite(ctx.emaF) && Number.isFinite(ctx.emaS) && ctx.emaF > ctx.emaS;
  const trendDn  = Number.isFinite(ctx.emaF) && Number.isFinite(ctx.emaS) && ctx.emaF < ctx.emaS;
  const macdUp   = Number.isFinite(ctx.macdNow) && ctx.macdNow > 0;
  const macdDn   = Number.isFinite(ctx.macdNow) && ctx.macdNow < 0;
  const rsiUp    = Number.isFinite(ctx.rsiVal) && ctx.rsiVal >= 50;
  const rsiDn    = Number.isFinite(ctx.rsiVal) && ctx.rsiVal < 50;
  const bull = (trendUp?1:0) + (macdUp?1:0) + (rsiUp?1:0);
  const bear = (trendDn?1:0) + (macdDn?1:0) + (rsiDn?1:0);
  if (bull >= 2 && bear < 2) return 'شراء';
  if (bear >= 2 && bull < 2) return 'بيع';
  return null;
}
...
function filteredSignal(tf,series,rsiArr,macdObj,atrArr,rows5Ref,rows30Ref,rows60Ref,piv,stochObj,bbObj){
  ...
  if (window.gsEntryPolicy==='2of3' && sig==='حيادي') { const qp=quickPolicySide(ctx); if(qp) sig=qp; }
...
async function runAnalysis(){
  try{
    ...
    // Quick Mode evaluation & application
    const indForMode = { atrPct: atrPct(atrArr?.[i], px), bb: bb?{upper:bb.up?.[i], lower:bb.dn?.[i], mid:bb.mid?.[i]}:null };
    const act = computeActiveModeForNow(indForMode);
    applyFlagsForMode(act.mode);
...
document.addEventListener('DOMContentLoaded', ()=>{
  ...
  if (wrap){
    wrap.addEventListener('click', (e)=>{
      ...
      // سنحدّث الواجهة بعد حساب الإشارات ليظهر السبب
      updateModeUI(GS_MODE, '');
      try{ runAnalysis(); }catch(e){}
    });
    // تهيئة أولية لشكل الأزرار
    updateModeUI(GS_MODE, '');
  }
});


// --- Init Mode Toggle (Fast / Safe / Auto) ---
function __initGsModeToggle(){
  ...
  if (!wrap.__wired){
    wrap.addEventListener('click', (e)=>{
      const btn = e.target.closest('button[data-mode]');
      if(!btn) return;
      GS_MODE = btn.dataset.mode;                // 'fast' | 'safe' | 'auto'
      localStorage.setItem(GS_MODE_KEY, GS_MODE);
      updateModeUI(GS_MODE, '');
      try{ runAnalysis(); }catch(e){}
    });
...
if (document.readyState === 'loading'){
  document.addEventListener('DOMContentLoaded', __initGsModeToggle);
} else {
  __initGsModeToggle();
}
