...
/* ====== PATCH: live price (worker-first, no-cache) + gentler spike handling ====== */
(function(){
  // مصادر السعر (الأولوية للـ worker)
  const LIVE_SOURCES_PATCH = [
    'https://workerjs.samer-mourtada.workers.dev/price',
    'https://gold-ticks.samer-mourtada.workers.dev/price'
  ];

  // --- replace fetchLivePrice ---
  window.fetchLivePrice = async function fetchLivePrice(){
    const TIMEOUT_MS = 2500;

    async function getOnce(url){
      // أضف t=timestamp لتفادي أي كاش وسيط
      ...
      try{
        ...
        if (ct.includes('json')) {
          ...
          if (Array.isArray(j) && Number.isFinite(j[0])) return +j[0];
          if (j && Number.isFinite(j.price)) return +j.price;
          if (j && Number.isFinite(j.close)) return +j.close;
        }
        // كـ fallback بسيط لو رجع نص فيه أرقام
    ...
    const vals = (await Promise.allSettled(LIVE_SOURCES_PATCH.map(getOnce)))
                  .filter(x=>x.status==='fulfilled')
                  .map(x=>x.value)
                  .filter(Number.isFinite);

    if (!vals.length) throw new Error('تعذّر جلب السعر الحي');

    // robust median (تنظيف outliers الطفيفة)
    const avg   = vals.reduce((a,b)=>a+b,0)/vals.length;
    const clean = vals.filter(v=>Math.abs(v-avg)/Math.max(1e-9,avg) < 0.015); // 1.5%
    const arr   = clean.length ? clean : vals;
    arr.sort((a,b)=>a-b);
  ...
  // --- replace refreshLive ---
  window.refreshLive = async function refreshLive(){
    try{
      const price = await Promise.resolve(window.fetchLivePrice());
      const nowMs = Date.now();
      if (typeof price === 'number' && isFinite(price)) {
        const prev = (typeof window.__livePrice === 'number' && isFinite(window.__livePrice)) ? window.__livePrice : null;

        // خفّفنا الفلترة: سجّل فقط بدون إيقاف التحديث
        if (prev != null) {
          const pct = Math.abs(price - prev) / Math.max(1e-9, Math.abs(prev));
          if (pct > 0.02) { try{ console.warn('Spike (ignored for UI stop):', pct); }catch(_){} }
        }

        // حدّث القيم العامة دومًا
        window.__livePrice  = price;
        window.__liveTimeMs = nowMs;
        // لوحـة النصيحة والرسم تعتمد على LAST_LIVE
        window.LAST_LIVE = { price, timeMs: nowMs };

        try { if (typeof paintLive === 'function') paintLive(price, nowMs); } catch(_){}
        try { if (typeof reprojectWithLive === 'function') reprojectWithLive(); } catch(_){}
      }
    } catch(e){
      try{ console.warn('Live error:', e); }catch(_){}
    }
  };
})();
