...
/* ===== PATCH: single-source live (worker-first, fallback gold, freshness-guard) ===== */
(function(){
  ...
  const FRESH_S = 3; // if worker tick older than 3s, fallback

  async function fetchJson(u){
    const url = u + (u.includes('?') ? '&' : '?') + 't=' + Date.now();
    const r = await fetch(url, { cache: 'no-store', headers: { 'cache-control': 'no-cache, no-store', 'accept': 'application/json' } });
    if (!r.ok) throw new Error('HTTP '+r.status);
    const ct = (r.headers.get('content-type')||'').toLowerCase();
    if (ct.includes('json')) return await r.json();
    // fallback: try parse as text number if provider misbehaves
    const raw = await r.text();
    const nums = (raw.match(/[+-]?\d+(?:\.\d+)?/g)||[]).map(Number).filter(Number.isFinite);
    if (nums.length) return { price: nums[nums.length-1] };
    throw new Error('bad upstream body');
  }

  function num(x){ const n = Number(x); return Number.isFinite(n) ? n : null; }

  // Override ONLY fetchLivePrice; leave everything else intact
  window.fetchLivePrice = async function fetchLivePrice(){
    const now = Date.now();
    try{
      const w = await fetchJson(WORKER_URL);
      const wp = num(w.price ?? w.close);
      const wt = num(w.ts ?? w.timeMs ?? w.timestamp);
      const fresh = (wp!=null) && (wt==null || (now - wt) <= FRESH_S*1000);
      if (fresh) return wp;
      // stale worker -> try gold-ticks
      const g = await fetchJson(GOLDTICKS_URL);
      const gp = num(g.price ?? g.close);
      if (gp!=null) return gp;
      // if worker had a price but stale, still use it as last resort
      if (wp!=null) return wp;
      throw new Error('no price');
    }catch(_){
      // final fallback to gold-ticks
      try{
        const g = await fetchJson(GOLDTICKS_URL);
        const gp = num(g.price ?? g.close);
        if (gp!=null) return gp;
...
/* ===== KPI-Live Hard Sync (add-only, safe) =====
   Ensures the numeric KPI on the page always mirrors __livePrice/LAST_LIVE,
   even if another loop fails to paint the label.
*/
(function(){
  function syncLiveLabel(){
    try{
      const el = document.getElementById('livePrice') 
                || document.querySelector('#livePrice, .live-price, [data-live-price]');
      const p = (typeof window.__livePrice === 'number' && isFinite(window.__livePrice)) 
                ? window.__livePrice 
                : (window.LAST_LIVE && typeof window.LAST_LIVE.price === 'number' ? window.LAST_LIVE.price : null);
      if (!el || p == null) return;
      // format with 3 decimals; adjust if you prefer
      const txt = (Math.round(p * 1000) / 1000).toFixed(3);
      if (el.textContent !== txt) el.textContent = txt;
    }catch(_){}
  }

  // keep KPI synced twice per second
  if (!window.__KPI_SYNC_LOOP__){
    window.__KPI_SYNC_LOOP__ = true;
    setInterval(syncLiveLabel, 500);
    // also sync after common UI events that rebuild the DOM
    ['load','pageshow','visibilitychange','hashchange','focus'].forEach(ev=>{
      addEventListener(ev, ()=>setTimeout(syncLiveLabel, 50), { passive:true });
    });
  }
})();


/* ==== GS EMERGENCY LIVE SYNC (embedded) ==== */
(function(){
  if (window.__GS_EMERGENCY_SYNC__) return; window.__GS_EMERGENCY_SYNC__ = true;

  const WORKER = 'https://workerjs.samer-mourtada.workers.dev/price';
  const GOLD   = 'https://gold-ticks.samer-mourtada.workers.dev/price';
  ...
  async function jget(u){
    ...
    try { if (ct.includes('json') || text.trim().startsWith('{')) return JSON.parse(text); } catch(_){}
    const m = text.match(/[+-]?\d+(?:\.\d+)?/g);
    return m ? { price: Number(m.pop()) } : {};
  }
  async function getLive(){
    try{
      const w = await jget(WORKER);
      const wp = num(w.price ?? w.close);
      const wt = num(w.ts ?? w.timeMs ?? w.timestamp);
      const fresh = wp != null && (wt==null || Date.now()-wt <= 3000);
      if (fresh) return wp;
      const g = await jget(GOLD);
      const gp = num(g.price ?? g.close);
      return gp ?? wp ?? null;
    }catch(_){ return null; }
  }

  function fmt(v){ try{return (Math.round(v*1000)/1000).toFixed(3);}catch(_){return String(v);} }
  function ensureKpi(){
    let el = document.querySelector('#livePrice, .live-price, [data-live-price]');
    if (el) return el;
    el = document.createElement('span');
    el.id = 'livePrice'; el.setAttribute('data-live-price','');
    const lbl = Array.from(document.querySelectorAll('*')).find(n => /السعر\s*الحي/i.test(n.textContent||''));
    if (lbl) lbl.insertAdjacentElement('afterend', el); else document.body.prepend(el);
    return el;
  }
  ...
  async function tick(){
    ...
    if (p==null) return;
    const now = Date.now();
    // وحّد الكرت والشارت على نفس الرقم
    window.__livePrice  = p;
    window.__liveTimeMs = now;
    window.LAST_LIVE    = { price: p, timeMs: now };

    try { if (typeof window.paintLive === 'function') window.paintLive(p, now); } catch(_){}
    try { if (typeof window.reprojectWithLive === 'function') window.reprojectWithLive(); } catch(_){}
  ...
  clearInterval(window.__GS_SYNC_IV__);
  window.__GS_SYNC_IV__ = setInterval(tick, 1000);
  tick();
})();
