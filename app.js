/* GoldSignals app.js — SSE + CSV/Stooq/OHLC loaders + live-merge + fixed CSV export (CRLF+BOM) */
...
function downloadMergedCsv(){
  ...
  const CRLF = '\r\n';
  let csv = 'Date,Open,High,Low,Close' + CRLF;
  for (const r of merged) {
    const dt = new Date(r.ts).toISOString();
    csv += `${dt},${r.open},${r.high},${r.low},${r.close}` + CRLF;
  }
  const BOM = '\ufeff';
