...
async function fetchLivePrice(){
  ...
  async function get(url){
    ...
    try{
      const r = await fetch(url+(url.includes('?')?'&':'?')+'t='+Date.now(),{cache:'no-store',mode:'cors', signal: ctl.signal, headers:{'cache-control':'no-cache, no-store'}});
    ...
    Promise.resolve(fetchLivePrice()).then(function(price){
      ...
      if (typeof price === 'number' && isFinite(price)) {
        ...
        if (prev != null) {
          ...
          if (pct > 0.007) { try{ console.warn('Spike filtered', pct); }catch(_){} /* keep painting to avoid line disappearance */ }
        }
        try { if (typeof paintLive === 'function') { paintLive(price, __now); } } catch(_) {}
        window.__livePrice = price;
        window.__liveTimeMs = __now;
        window.LAST_LIVE = { price, timeMs: __now };
