...
async function fetchCsvText(url){
  ...
  const r=await fetch(full); if(!r.ok) throw new Error('CSV HTTP '+r.status); return await r.text();
}
...
async function checkCsvAge(url){
  try{ const txt = await fetchCsvText(url); const lines = txt.trim().split(/\r?\n/).filter(Boolean); let d=null;
    if(lines.length>1){ const p=lines[1].split(','); const v=p[0]||p[1]; if(v && !isNaN(Date.parse(v))) d=Date.parse(v); }
    elCsvAge.textContent = d? `CSV: ${Math.round((Date.now()-d)/3600000)}h` : 'CSV: —';
  }catch{ elCsvAge.textContent='CSV: —'; }
}

function setConnStatus(text,cls){ elConn.textContent=`الاتصال: ${text}`; elConn.className='status '+(cls||''); }
function startSSE(){
...
function handleLiveTick(price, timeMs){
  ...
  elLivePrice.textContent=nf2.format(price); elLiveTime.textContent=fmtLocalDateTime(timeMs);
  reprojectWithLive();
}
...
function fillTable(series, rsiArr, mac){
  ...
  for(const r of lastN.reverse()){
    ...
    tr.innerHTML=`<td>${new Date(r.ts).toLocaleDateString('en-CA')}</td>
      ...
      <td>${elSummaryText.textContent}</td>
...
async function runAnalysis(){
  ...
  try{ const base=await loadBaseSeries(currentTF); setConnStatus(__evtSrc?'SSE متصل':'جاهز','ok'); buildAndRender(base); }
  catch(e){ setConnStatus('فشل التحميل','bad'); console.error(e); }
}
...
setInterval(()=>{ if(!__evtSrc){ /* optional polling hook */ } }, LIVE_REFRESH_SEC*1000);
