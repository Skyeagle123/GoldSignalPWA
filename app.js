...
// === GS market-mode hookup (auto obeys trend/range) ===
window.addEventListener('gs:market:metrics', (e)=>{
  try { window.__gsMarketMetrics = e.detail || {}; } catch {}
});

function _getGsMetrics() {
  try {
    const mm = window.__gsMarketMetrics || {};
    const bb = (mm.bbPct ?? window.gs?.market?.bbPct);
    const atr= (mm.atrPct ?? window.gs?.market?.atrPct);
    const md = (mm.mode  ?? window.gs?.market?.mode);
    return { bbPct: bb, atrPct: atr, mode: md };
  } catch { return {}; }
}

function marketModeFromMetrics() {
  const { bbPct, atrPct, mode } = _getGsMetrics();
  if (mode === 'trend' || mode === 'range') return mode;
  if (Number.isFinite(bbPct) && Number.isFinite(atrPct)) {
...
function filteredSignal(tf,series,rsiArr,macdObj,atrArr,rows5Ref,rows30Ref,rows60Ref,piv,stochObj,bbObj){
  ...
  // ---- NEW: follow market mode (trend/range) from metrics
  const mMode = marketModeFromMetrics(); // 'trend' | 'range' | null
  if (mMode === 'range') {
