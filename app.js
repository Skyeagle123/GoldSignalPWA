...
/* ===== Mini Stats: BB% and ATR% (safe add) ===== */

function gs_pickCandles() {
  return (window.candles)
      || (window.chartCandles)
      || (window.series && window.series.candles)
      || null;
}

function gs_atr14(candles) {
  ...
  for (let i = 1; i < candles.length; i++) {
    const hi = H(candles[i]);
    const lo = L(candles[i]);
    const cprev = C(candles[i-1]);
    if ([hi, lo, cprev].some(v => v == null)) return null;
    const tr = Math.max(hi - lo, Math.abs(hi - cprev), Math.abs(lo - cprev));
    trs.push(tr);
  }
  if (trs.length < 14) return null;
  const last14 = trs.slice(-14);
  const atr = last14.reduce((a,b)=>a+b,0) / 14;
  return atr;
}

function gs_bollPctB(candles, len=20, k=2) {
  ...
  const variance = last.reduce((a,b)=>a + Math.pow(b-mean,2), 0) / len;
  ...
  const pctB = ((c - lower) / denom) * 100;
  return pctB;
}

function gs_getMiniStatsNode() {
  ...
  el.textContent = 'BB%: — • ATR%: —';
  const priceBox = document.querySelector('.live-price, [data-live-price]');
  if (priceBox && priceBox.parentElement) {
    priceBox.parentElement.insertBefore(el, priceBox);
  } else {
...
function gs_updateMiniStats() {
  const candles = gs_pickCandles();
  const el = gs_getMiniStatsNode();
  if (!candles || !el) return;

  const atr = gs_atr14(candles);
  const pctB = gs_bollPctB(candles);

  const C = (c) => c.c ?? c.close;
  const last = Array.isArray(candles) && candles.length ? C(candles[candles.length-1]) : null;
  const atrPct = (atr != null && last) ? (atr / last * 100) : null;
...
window.addEventListener('load', () => {
  gs_updateMiniStats();
  setTimeout(gs_updateMiniStats, 1000);
});

window.gs_updateMiniStats = gs_updateMiniStats;
