...
/* === Sync UI with market metrics (mode + BB/ATR) === */
(function(){
  const fmt = v => (v==null || Number.isNaN(v)) ? "—" : Number(v).toFixed(2);
  function applyModeToUI(mode, bb, atr){
    const stateRow = document.querySelector('[data-gs="market-state-row"]');
    if (stateRow){
      let stats = stateRow.querySelector('.hint-stats');
      if (!stats){
        stats = document.createElement('div');
        stats.className = 'hint hint-stats';
        stateRow.appendChild(stats);
      }
      stats.textContent = `BB%: ${fmt(bb)} • ATR%: ${fmt(atr)}`;
      const badge = document.getElementById('marketModeLabel') || document.getElementById('mtfBadge');
      if (badge){ badge.textContent = (mode === 'trend') ? 'ترند' : (mode === 'range') ? 'رينج' : 'غير معلوم'; }
      // toggle buttons if you have data-mode attributes
  ...
  function onMetrics(e){
    ...
    try{ (window.gs = window.gs || {}).market = Object.assign((window.gs.market||{}), {mode, bbPct:bb, atrPct:atr}); }catch(e){}
    applyModeToUI(mode, bb, atr);
  }
  window.addEventListener('gs:market:metrics', onMetrics, {passive:true});
  setTimeout(() => {
    const gs = window.gs || {}; const m = gs.market?.mode;
    if (m) applyModeToUI(m, gs.market?.bbPct ?? gs.market?.bbPerc, gs.market?.atrPct ?? gs.market?.atrPerc);
  }, 400);
})();
