...
/* === ADD-ONLY PATCH: Import/Export wiring + robust CSV/XLS export for mobile Office === */
(function(){
  const $ = (id)=>document.getElementById(id);

  function localMergeLive(series, tf, live){
    try{
      if(!Array.isArray(series) || !series.length || !live) return series;
      const ms = (tf||5)*60*1000;
      const b  = Math.floor((live.timeMs || Date.now())/ms)*ms;
      const out  = series.slice();
      const last = Object.assign({}, out[out.length-1]);
      if (b === last.ts){
  ...
  function buildMergedCsvText(){
    const tf = (window.__cache?.tf) || (typeof window.currentTF!=='undefined' ? window.currentTF : 5);
    let base = window.__seriesForExport || window.__lastBaseSeries || window.__cache?.series || [];
    if (!Array.isArray(base)) base = [];
    let merged = (typeof window.mergeLiveIntoSeries === 'function')
                 ? window.mergeLiveIntoSeries(base, tf, window.LAST_LIVE)
                 : localMergeLive(base, tf, window.LAST_LIVE);
    ...
    const rows = merged.map(r=>{
      ...
      return [date, r.open, r.high, r.low, r.close].map(q).join(',');
    });
    const BOM = '\uFEFF';
    return BOM + 'sep=,' + CRLF + header + CRLF + rows.join(CRLF) + CRLF;
  }

  function downloadMergedCsv(){
    let series = window.__seriesForExport || window.__lastBaseSeries || window.__cache?.series;
    if(!series || !series.length){ alert('ما في بيانات للتصدير. شغّل التحليل أو استورد CSV.'); return; }
    const csv = buildMergedCsvText();
    const blob = new Blob([csv], { type: 'text/csv;charset=utf-8' });

    const now = new Date(); const pad=(n)=>String(n).padStart(2,'0');
    const fname = `XAUUSD_${(window.__cache?.tf||window.currentTF||5)}min_merged_${now.getFullYear()}${pad(now.getMonth()+1)}${pad(now.getDate())}_${pad(now.getHours())}${pad(now.getMinutes())}${pad(now.getSeconds())}.csv`;
  ...
  function downloadMergedXls(){
    ...
    const now = new Date(); const pad=(n)=>String(n).padStart(2,'0');
    const fname = `XAUUSD_${(window.__cache?.tf||window.currentTF||5)}min_merged_${now.getFullYear()}${pad(now.getMonth()+1)}${pad(now.getDate())}_${pad(now.getHours())}${pad(now.getMinutes())}${pad(now.getSeconds())}.xls`;
  ...
  function wire(){
    const exp = $('btnExportCsv');
    const imp = $('btnImportCsv');
    const file= $('hiddenCsvFile');

    if(exp && !exp.__wired){
      const isAndroid = /Android/i.test(navigator.userAgent);
      exp.addEventListener('click', ()=> {
        if (isAndroid) downloadMergedXls();
        else downloadMergedCsv();
      });
      exp.__wired = true;
    }
    if(imp && !imp.__wired){
      imp.addEventListener('click', ()=> file && file.click());
      imp.__wired = true;
    }
  ...
  if (typeof _render === 'function'){
    window.renderTradeChart = function(series, lines){
      try{ window.__seriesForExport = Array.isArray(series)? series.slice(): null; }catch{}
      return _render.apply(this, arguments);
    };
  }
  wire(); document.addEventListener('DOMContentLoaded', wire);
})();
