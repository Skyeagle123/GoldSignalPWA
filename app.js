...
/* ========= GoldSignals Hotfix Patch (add-only) =========
   - Sync chart with live KPI: mirror __livePrice into LAST_LIVE and reproject when it changes.
   - Auto Telegram: watch #summaryText and POST rich payload (entry/tp1/tp2/sl) to the worker /alert.
*/
(function(){
  try{
    // --- 1) Keep chart in sync with live KPI ---
    let __lastLiveSeen = { price: null, t: null };
    function bumpLive(){
      try{
        const p = (typeof window.__livePrice === 'number' && isFinite(window.__livePrice)) ? window.__livePrice : null;
        const tm = (typeof window.__liveTimeMs === 'number' && isFinite(window.__liveTimeMs)) ? window.__liveTimeMs : null;
        if (p==null || tm==null) return;
        const changed = (p !== __lastLiveSeen.price) || (tm !== __lastLiveSeen.t);
        if (!changed) return;
        __lastLiveSeen = { price: p, t: tm };
        // expose LAST_LIVE so reprojectWithLive can merge immediately
        window.LAST_LIVE = { price: p, timeMs: tm };
        if (typeof window.reprojectWithLive === 'function') {
          try { window.reprojectWithLive(); } catch(_){}
        }
      }catch(_){}
    }
    setInterval(bumpLive, 400);

    // --- helpers used by Telegram bridge ---
    function tfShort(){
      try{
    ...
    function parseSideFromText(t){
      if (!t) return null;
      if (/شراء/.test(t)) return 'BUY';
      if (/بيع/.test(t)) return 'SELL';
      return null;
    }
    function parseEntryFromText(t){
      const m = /سعر الدخول:\s*([\d.]+)/.exec(t || '');
      return m ? parseFloat(m[1]) : null;
    }

    // --- 2) Auto Telegram from summaryText ---
    const WORKER_ALERT = "https://workerjs.samer-mourtada.workers.dev/alert";
    let __dedup = null;
    async function sendAlertFromSummary(t){
      try{
        const text = String(t||'').trim();
        if (!text || /لا توجد نصيحة|مرفوض|حيادي/.test(text)) return;
        const side = parseSideFromText(text);
        if (!side) return;
        const entry = parseEntryFromText(text);
        const id = side + '|' + tfShort() + '|' + (Number.isFinite(entry)?entry:'na');
        if (__dedup === id) return;
        __dedup = id;

        // compute targets (absolute) if helper exists
        let tp1=null, tp2=null, sl=null;
        try{
          if (typeof window.gsComputeTargets === 'function') {
            const tObj = window.gsComputeTargets(entry ?? window.__livePrice, side);
            tp1 = tObj.tp1; tp2 = tObj.tp2; sl = tObj.sl;
          }
        }catch(_){}
        ...
        const payload = {
          side,
          tf: tfShort(),
          entry: Number.isFinite(entry) ? entry : (typeof window.__livePrice==='number'?window.__livePrice:null),
          tp1, tp2, sl,
          filtersRejected: false
        };
      ...
      }catch(e){ console.warn('[patch] sendAlertFromSummary failed', e); }
    }
    ...
    function wireObserver(){
      try{
        ...
        const mo = new MutationObserver(()=> sendAlertFromSummary(target.textContent));
        mo.observe(target, { childList: true, subtree: true, characterData: true });
        return true;
      }catch(_){ return false; }
    }
    if (!wireObserver()){
      document.addEventListener('DOMContentLoaded', wireObserver, { once:true });
      window.addEventListener('load', wireObserver, { once:true });
    }
  }catch(e){ console.warn('[patch] init failed', e); }
})();
// ========= End Hotfix Patch =========
