...
const DEFAULT_5M_CSV   = './XAUUSD_5min.csv';   // مهم: ./ لضمان المسار على صفحات GitHub
const TABLE_ROWS       = 80;
...
const fmtLocal = (ts) => {
  try{
    return new Date(ts).toLocaleString([], {hour12:false, timeZone:tz})
      .replace(',', '');
  }catch{ return String(ts); }
};

/*--------- حالة الإطار الزمني ---------*/
let currentTF = 5; // 5, 30, 60, 1440
...
async function fetchCsv(url){
  ...
  const full = u.startsWith('http') ? u : `${u}?t=${Date.now()}`; // كسر كاش
...
function paintLive(price, ts){
  ...
  if (elLiveTime  && ts!=null)               elLiveTime.textContent  = fmtLocal(ts);
}
...
async function runAnalysis(){
  try{
    ...
    const rows5 = await fetchCsv(csvUrl);   // صفوف 5 دقائق
    if (!rows5.length) throw new Error('ملف CSV فارغ');

    // سلاسل مجمّعة
    const series5   = rows5;
    const series30  = aggregateOHLC(rows5, 30);
    const series60  = aggregateOHLC(rows5, 60);
    const seriesDay = aggregateOHLC(rows5, 1440);

    // اختيار الإطار الزمني
    let series = series5;
    if (currentTF===30)   series = series30;
    if (currentTF===60)   series = series60;
    if (currentTF===1440) series = seriesDay;
    ...
    const piv = calcPivots(seriesDay);
    paintPivots(piv);
...
async function refreshLive(){
  try{
    ...
    if (j && j.ok && Number.isFinite(j.price)){
      // حاول نقرأ TS إن وجد، أو استخدم الآن
      const ts = j.ts ?? (j.isoTime ? Date.parse(j.isoTime) : Date.now());
      paintLive(j.price, ts);
    }
...
/*--------- مُلتقط أخطاء للتشخيص ---------*/
window.addEventListener('error', (e) => {
  console.error('JS Error:', e.message);
  alert('خطأ JavaScript: ' + e.message);
});
