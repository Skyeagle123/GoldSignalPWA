...
/* === GS: worker-only sync (safe minimal) === */
(function(){
  if (window.__GS_WORKER_SYNC_SAFE__) return; window.__GS_WORKER_SYNC_SAFE__ = true;
  ...
  async function tick(){
    try{
      ...
      // update KPI (card)
      setKpi(price, ts);
      // redraw chart if functions exist
      try{ if (typeof paintLive === 'function') paintLive(price, ts); }catch(_){}
      try{ if (typeof reprojectWithLive === 'function') reprojectWithLive(); }catch(_){}
    }catch(_){}
  }

  // if DOM is rebuilt, re-apply current value
  try{
    const mo = new MutationObserver(()=> {
  ...
  clearInterval(window.__GS_WORKER_SYNC_IV__);
  window.__GS_WORKER_SYNC_IV__ = setInterval(tick, 1000);
  tick();
})();
