...
/* === GS hook (ADD-ONLY): expose candles to metrics === */
(function(){
  function publishCandles(){
    try{
      var GSref = (typeof window.GS!=='undefined') ? window.GS : null;
      var c = (GSref && GSref.state && Array.isArray(GSref.state.candles) && GSref.state.candles) ||
              (GSref && Array.isArray(GSref.candles) && GSref.candles) ||
              (Array.isArray(window.candles) && window.candles) ||
              (Array.isArray(window.__CANDLES__) && window.__CANDLES__) || null;
      if (Array.isArray(c) && c.length){
        window.__CANDLES__ = c;
      }
    }catch(e){ /* no-op */ }
  }
  // run now and keep it fresh every 2s
  if (document.readyState === 'loading'){
    document.addEventListener('DOMContentLoaded', publishCandles, {once:true});
  } else {
    publishCandles();
  }
  setInterval(publishCandles, 2000);
})();
