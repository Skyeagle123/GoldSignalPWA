...
/* ——— ضبط خطوة السعر (Tick) للذهب ——— */
const TICK = 0.01;
const roundTick = v => (Number.isFinite(v) ? Math.round(v / TICK) * TICK : v);
...
/* ——— شارة توكيد متعدد الأطر (MTF) ——— */
function setMtfBadge(ok){
  const b = document.getElementById('mtfBadge');
  if(!b) return;
  b.textContent = ok ? 'MTF ✅' : 'MTF ❌';
  b.style.padding = '2px 6px';
  b.style.borderRadius = '8px';
  b.style.fontSize = '.85em';
  b.style.background = ok ? '#d1fae5' : '#fee2e2';
  b.style.border = '1px solid ' + (ok ? '#10b981' : '#ef4444');
}
...
function buildAdvice(tf,series,rsiArr,macdObj,piv,live,atrArr,rows5,rows30,rows60,stoch,bb){
  ...
  const mkLines=(side)=>{ let entry=(side==='شراء')?Math.max(nowPx,Number.isFinite(emaS)?emaS:nowPx):Math.min(nowPx,Number.isFinite(emaS)?emaS:nowPx);
    ...
    const tp2=(side==='شراء')?entry+TP2_ATR_MULT*atrV:entry-TP2_ATR_MULT*atrV; entry=roundTick(entry); const slR=roundTick(sl), tp1R=roundTick(tp1), tp2R=roundTick(tp2); return {entry, sl:slR, tp1:tp1R, tp2:tp2R}; };
...
async function runAnalysis(){
  try{
    ...
    try{ setMtfBadge(((tf!==5) || !MTF_CONFIRM || strongMTFConfirm(rows30,rows60))); }catch(_){}
try{ setMtfBadge(((currentTF!==5) || !MTF_CONFIRM || strongMTFConfirm(rows30,rows60))); }catch(_){}
    ...
    try {
      ...
      const payload = {
       ...
       tf: (typeof currentTF !== "undefined" ? currentTF : tfShort() || "30m"),
        ...
        price: (Number.isFinite(entry) ? entry : (typeof __livePrice !== "undefined" ? __livePrice : (L && Number.isFinite(L.entry) ? L.entry : null))),
...
/* ——— توضيح حالة مفاتيح التعطيل للفلاتر ——— */
function paintToggleNotes(){
  const noteNy = document.getElementById('nyFilterNote');
  const notePv = document.getElementById('pivotFilterNote');
  if(noteNy) noteNy.style.color = (elToggleNyHours?.checked) ? 'lawngreen' : '';
  if(notePv) notePv.style.color = (elTogglePivotFilter?.checked) ? 'lawngreen' : '';
}
try{
  elToggleNyHours?.addEventListener('change', paintToggleNotes);
  elTogglePivotFilter?.addEventListener('change', paintToggleNotes);
}catch(_){}
window.addEventListener('load', function(){
  paintToggleNotes();
  window.__liveAgeOkMs = 3000; // حساسية تأخر اللايف: 3 ثواني
}, {once:true});
