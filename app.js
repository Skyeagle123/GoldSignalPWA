...
/* === GS Dedupe v3 — keep only the top BB/ATR line === */
(function(){
  const TOP_ID = 'gsMarketMetricsLine';
  function isTopNode(el){ return el && (el.id === TOP_ID || (el.closest && el.closest('#'+TOP_ID))); }
  function looksLikeMetric(el){
    const t = (el.textContent||'').replace(/\s+/g,' ').trim();
    return t && (/حالة السوق/.test(t) && /(BB%|ATR%)/.test(t) || (/^—\s*%?BB%/.test(t) && /ATR%/.test(t)));
  }
  function sweep(){
    document.querySelectorAll('body *').forEach(el=>{
      if (isTopNode(el)) return;
      const inHeader = el.closest && el.closest('header');
      if (!inHeader && looksLikeMetric(el)) el.remove();
    });
  }
  new MutationObserver(()=>setTimeout(sweep,50)).observe(document.documentElement,{childList:true,subtree:true});
  document.addEventListener('DOMContentLoaded',()=>setTimeout(sweep,100));
  window.addEventListener('gs:market:metrics',()=>setTimeout(sweep,50),{passive:true});
  window.addEventListener('gs:state:changed',()=>setTimeout(sweep,50),{passive:true});
