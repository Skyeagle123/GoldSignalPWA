...
/* === GS candles bridge: ensure indicators always receive candles === */
(function(){
  function pushCandles(series){
    try{
      const s = Array.isArray(series) ? series : (
        (window.__seriesForExport && Array.isArray(window.__seriesForExport)) ? window.__seriesForExport :
        (window.gs && (gs.candles || gs.series || gs.data)) || []
      );
      if (!s || !s.length) return;
      window.__seriesForExport = s;
      window.dispatchEvent(new CustomEvent('gs:candles:updated',{ detail:{ candles: s } }));
    }catch(e){}
  }
  window.addEventListener('gs:chart:rendered', e => pushCandles(e.detail?.series), {passive:true});
  window.addEventListener('gs:state:changed',   () => pushCandles(), {passive:true});

  let lastLen = 0, lastTf = null;
  setInterval(() => {
    try{
      const g = window.gs || {};
      const s = window.__seriesForExport || g.candles || g.series || g.data || null;
      const currLen = Array.isArray(s) ? s.length : 0;
      const tf = g.tf || document.querySelector('.tabs .t.active')?.id || null;
      if (currLen && (currLen !== lastLen || tf !== lastTf)) {
        lastLen = currLen; lastTf = tf;
        pushCandles(s);
      }
    }catch(e){}
  }, 1000);
  ...
  if (document.readyState === 'loading') {
    document.addEventListener('DOMContentLoaded', () => { setTimeout(pushCandles, 200); setTimeout(pushCandles, 1000); }, {once:true});
  } else {
    setTimeout(pushCandles, 200);
    setTimeout(pushCandles, 1000);
  }

  try{
    const orig = window.renderTradeChart;
