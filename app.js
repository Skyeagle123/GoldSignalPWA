...
(function(){
  ...
  function inferStep(candles){
    // خذ فروقات الأزمنة الإيجابية واختر الوسيط (median) كخطوة متوقعة
    const ts = candles.map(toTs).filter(Number.isFinite);
    if (ts.length < 3) return 300000; // 5m افتراضي
    const diffs = [];
  ...
  function fillGaps(candles){
    ...
    for (let i=1;i<arr.length;i++){
      ...
      if (Number.isFinite(prevTs) && Number.isFinite(currTs)) {
        ...
        if (gap > step*1.6) {
          // عدّة شموع ناقصة: عبّيها بشموع مسطّحة من سعر الإغلاق السابق
