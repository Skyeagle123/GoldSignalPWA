...
/* ===== MERGE COMPAT (from app-26.js UI toggles) ===== */
(function(){
  const $ = (id)=>document.getElementById(id);
  ...
  if (typeof window.USE_RSI === 'undefined') window.USE_RSI = true;
  if (typeof window.USE_MACD === 'undefined') window.USE_MACD = true;
  if (typeof window.USE_EMA_TREND === 'undefined') window.USE_EMA_TREND = true;

  const __orig_loadSettings = (typeof loadSettings==='function') ? loadSettings : null;
  window.loadSettings = function(){
    if (__orig_loadSettings) __orig_loadSettings();
    const isAuto = !!(elAutoInd && elAutoInd.checked);
    const manualRSI  = !!(elUseRSI && elUseRSI.checked);
    const manualMACD = !!(elUseMACD && elUseMACD.checked);
    const manualEMA  = !!(elUseEMA && elUseEMA.checked);

    if (!isAuto){
      window.USE_RSI = manualRSI;
      window.USE_MACD = manualMACD;
      window.USE_EMA_TREND = manualEMA;
    } else {
      try{
        const series = (window.__lastSeriesForChart || window.__lastBaseSeries || null);
        if (Array.isArray(series) && series.length>30){
          const macObj = macd(series, EMA_FAST||12, EMA_SLOW||26, 9);
          const i = series.length-1;
          const emaUp = Number.isFinite(macObj.emaF[i]) && Number.isFinite(macObj.emaS[i]) && macObj.emaF[i] > macObj.emaS[i];
          const emaDn = Number.isFinite(macObj.emaF[i]) && Number.isFinite(macObj.emaS[i]) && macObj.emaF[i] < macObj.emaS[i];
          const trending = emaUp || emaDn;
          window.USE_EMA_TREND = trending;
          window.USE_MACD = true;
          const atrArr = atr(series, ATR_PERIOD||14);
          const last = series[i]?.close || 0;
          const atrP = (last && atrArr?.[i]) ? (atrArr[i]/last) : 0;
          const choppy = atrP >= 0.003;
          window.USE_RSI = choppy;
        } else {
          window.USE_RSI = true; window.USE_MACD = true; window.USE_EMA_TREND = true;
        }
      }catch(e){ window.USE_RSI = true; window.USE_MACD = true; window.USE_EMA_TREND = true; }
      if (elUseRSI)  elUseRSI.checked  = window.USE_RSI;
      if (elUseMACD) elUseMACD.checked = window.USE_MACD;
      if (elUseEMA)  elUseEMA.checked  = window.USE_EMA_TREND;
    }
  };

  const __orig_classifyFinal = (typeof classifyFinal==='function') ? classifyFinal : null;
  ...
  const __orig_paintIndicators = (typeof paintIndicators==='function') ? paintIndicators : null;
  window.paintIndicators = function(rsiVal,macdVal,emaFv,emaSv,stK,stD,bbMid,bbUp,bbDn){
    if (__orig_paintIndicators) __orig_paintIndicators(rsiVal,macdVal,emaFv,emaSv,stK,stD,bbMid,bbUp,bbDn);
    const show = (id,flag)=>{ const el=document.getElementById(id); if(!el) return;
      const node=el.parentElement||el; node.style.display = flag?'inline-block':'none'; };
  ...
  window.fetchCsv = async function(url){
    if (__orig_fetchCsv){
      try{ const rows=await __orig_fetchCsv(url); if(rows?.length) return rows; }catch(e){}
    }
    const FALLBACK = `Date,Time,Close
...
2025-09-30,13:55,4040.10`;
    const preferred = (url&&url.trim())?url.trim():(typeof DEFAULT_5M_CSV!=='undefined'?DEFAULT_5M_CSV:'XAUUSD_5min.csv');
    const base = location.pathname.replace(/\/[^/]*$/,'/');
    const candidates = [preferred, './'+preferred, base+preferred, base+'XAUUSD_5min.csv', base+'data/XAUUSD_5min.csv', 'XAUUSD_5min.csv', './XAUUSD_5min.csv'];
  ...
  [elUseRSI, elUseMACD, elUseEMA, elAutoInd].forEach(el=>{
    if(!el || el.__wired) return;
    el.addEventListener('change', ()=>{ try{ if (typeof runAnalysis==='function') runAnalysis(); }catch(e){} });
    el.__wired=true;
  });
})();


/* ===== AUTO SYNC PATCH EMBEDDED ===== */

/* Auto indicators UI sync (tiny patch) */
(function(){
  const $ = id => document.getElementById(id);
  ...
  function syncFromState(){
    // if state vars exist, mirror them to checkboxes
    if (typeof window.USE_RSI        !== 'undefined' && elRSI)  elRSI.checked  = !!window.USE_RSI;
    if (typeof window.USE_MACD       !== 'undefined' && elMACD) elMACD.checked = !!window.USE_MACD;
    if (typeof window.USE_EMA_TREND  !== 'undefined' && elEMA)  elEMA.checked  = !!window.USE_EMA_TREND;
    if (typeof window.USE_STOCH      !== 'undefined' && elSTO)  elSTO.checked  = !!window.USE_STOCH;
    if (typeof window.USE_BB         !== 'undefined' && elBB)   elBB.checked   = !!window.USE_BB;
  }

  function recalcThenSync(){
    try { if (typeof loadSettings==='function') loadSettings(); } catch(e){}
    try { if (typeof runAnalysis==='function') runAnalysis(); else if (typeof run==='function') run(); } catch(e){}
    syncFromState();
  }

  if (elAuto && !elAuto.__wiredSync){
    elAuto.addEventListener('change', ()=>{
      if (elAuto.checked){
        // turn Auto ON: let the app decide, then mirror to UI
        recalcThenSync();
      } else {
        // Auto OFF: keep current manual state; no-op
      }
    });
    elAuto.__wiredSync = true;
  }

  // Also sync once on load (for mobile refresh)
...
/* ===== ENSURE SERIES CACHE + AUTO UI SYNC (v2) ===== */
(function(){
  const $ = id => document.getElementById(id);

  // Wrap fetchCsv so we always cache series for auto-indicators
  ...
  // Re-run + sync when Auto toggled (again, in case previous hook didn't fire)
  const elAuto = $('autoInd');
  function triggerRun(){
    try { if (typeof loadSettings==='function') loadSettings(); } catch(e){}
    try { if (typeof runAnalysis==='function') runAnalysis(); else if (typeof run==='function') run(); } catch(e){}
  ...
  function syncFromState(){
    ...
    map.forEach(([id,key])=>{
      ...
      if (!el) return;
      if (key in window) el.checked = !!window[key];
    });
  }
  if (elAuto && !elAuto.__wired_v2){
    elAuto.addEventListener('change', ()=>{
      triggerRun();
      setTimeout(syncFromState, 150);
    });
    elAuto.__wired_v2 = true;
  }

  // Also sync shortly after first successful run
  document.addEventListener('DOMContentLoaded', ()=>{
...
/* ===== FINAL AUTO-BY-MARKET BOOTSTRAP (v3) ===== */
(function(){
  const $ = id => document.getElementById(id);

  // 1) Define sane defaults once
  if (typeof window.USE_RSI === 'undefined') window.USE_RSI = true;
  if (typeof window.USE_MACD === 'undefined') window.USE_MACD = true;
  if (typeof window.USE_EMA_TREND === 'undefined') window.USE_EMA_TREND = true;
  if (typeof window.USE_STOCH === 'undefined') window.USE_STOCH = false;
  if (typeof window.USE_BB === 'undefined') window.USE_BB = false;

  // 2) Always cache last series when CSV loads
  const __old_fetchCsv = (typeof fetchCsv==='function') ? fetchCsv : null;
  if (__old_fetchCsv){
    window.fetchCsv = async function(url){
      const rows = await __old_fetchCsv(url);
  ...
  function computeRegime(series){
    ...
    // use existing indicator functions from the app
    try{
      const EMA_FASTv = (typeof EMA_FAST!=='undefined')?EMA_FAST:12;
      const EMA_SLOWv = (typeof EMA_SLOW!=='undefined')?EMA_SLOW:26;
      const ATR_PERIODv = (typeof ATR_PERIOD!=='undefined')?ATR_PERIOD:14;
      const m = macd(series, EMA_FASTv, EMA_SLOWv, 9);
      const i = series.length-1;
  ...
  // 3) Wire Auto checkbox
  const auto = $('autoInd');
  if (auto && !auto.__wired_final){
    ...
    auto.__wired_final = true;
  }

  // 4) First load bootstrap (after DOM + after first CSV load)
  document.addEventListener('DOMContentLoaded', ()=>{
    setTimeout(()=>{
      const auto = $('autoInd');
      if (auto && auto.checked){
        ...
        // manual: mirror current UI to state once
        ['useRSI','useMACD','useEMA','useStoch','useBB'].forEach(id=>{
          const el=document.getElementById(id);
          if (!el) return;
          if      (id==='useRSI')  window.USE_RSI = !!el.checked;
          else if (id==='useMACD') window.USE_MACD = !!el.checked;
          else if (id==='useEMA')  window.USE_EMA_TREND = !!el.checked;
          else if (id==='useStoch')window.USE_STOCH = !!el.checked;
          else if (id==='useBB')   window.USE_BB = !!el.checked;
        });
      }
      runNow();
    }, 400);
...
/* =========================
   APP FINAL PATCH (Auto-by-market + UI sync + state bootstrap)
   This block is self-contained and safe to append at the end of the bundle.
   ========================= */
(function(){
  const $ = (id)=>document.getElementById(id);

  // ---- 0) Safe defaults so state is never undefined
  if (typeof window.USE_RSI === 'undefined') window.USE_RSI = true;
  if (typeof window.USE_MACD === 'undefined') window.USE_MACD = true;
  if (typeof window.USE_EMA_TREND === 'undefined') window.USE_EMA_TREND = true;
  if (typeof window.USE_STOCH === 'undefined') window.USE_STOCH = false;
  ...
  // ---- 1) Ensure latest CSV series cached for Auto logic
  const __orig_fetchCsv = (typeof fetchCsv==='function') ? fetchCsv : null;
  if (__orig_fetchCsv && !fetchCsv.__wrapped_final){
    window.fetchCsv = async function(url){
      const rows = await __orig_fetchCsv(url);
      try{ if (Array.isArray(rows) && rows.length) window.__lastSeriesForChart = rows; }catch(e){}
      return rows;
    };
    window.fetchCsv.__wrapped_final = true;
  }

  // ---- 2) Auto-by-market decision using existing indicator funcs
  function computeRegime(series){
    if (!Array.isArray(series) || series.length < 30) return {trend:'unknown', vol:'normal'};
    try{
      ...
      const last = series[i]?.close || 0;
      const ap = (last && aArr?.[i]) ? aArr[i]/last : 0;
      const vol = ap >= 0.003 ? 'high' : (ap <= 0.0008 ? 'low' : 'normal'); // 0.3% / 0.08%
      return {trend, vol};
  ...
  // ---- 3) Respect toggles inside classification and chips (if app lacked it)
