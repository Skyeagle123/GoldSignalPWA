...
/* ===== [GS] Mini-stats: place between 'حالة السوق' and 'Auto indicators' (Option 2) ===== */
(function(){
  const GS_MINI_ID = 'gs-mini-stats';
  ...
  function ensureMiniHost() {
    // If our node already exists, just show it
    let el = document.getElementById(GS_MINI_ID);
    if (el) { el.style.display=''; return el; }

    // Create a fresh line and place it right after "حالة السوق"
    const marketLabel = findByText('حالة السوق');
    el = document.createElement('div');
    el.id = GS_MINI_ID;
    ...
    // Option 2: strictly between "حالة السوق" and "Auto indicators"
    let placed = false;
    if (marketLabel) placed = insertAfter(marketLabel, el);
    if (!placed) {
      // fallback to top of indicators/summary/panel
      const preferred =
        document.querySelector('.indicators') ||
        document.querySelector('.summary') ||
        document.querySelector('.panel, .card');
  ...
  // Remove any BB% duplicates inside the "السعر الحي" section
  function removeLivePriceDuplicates(){
    const findByText = (txt)=>{
      const nodes = Array.from(document.querySelectorAll('h1,h2,h3,h4,div,span,p,label,small,strong,b'));
      return nodes.find(n => (n.textContent||'').trim().includes(txt)) || null;
    };
    const live = findByText('السعر الحي');
    if (!live) return;
    const box = live.closest('*') || live.parentElement;
    if (!box) return;
    box.querySelectorAll('div,span,p,small').forEach(n=>{
      if (n.id === GS_MINI_ID) return;
      if (/BB%\s*:/.test((n.textContent||''))) n.remove();
    });
  }

  // Wrap existing updater to also enforce placement & dedupe
  const prev = window.gs_updateBBATRLine;
  window.gs_updateBBATRLine = function(){
    try { ensureMiniHost(); } catch(e){}
    try { removeLivePriceDuplicates(); } catch(e){}
