/* GoldSignals app.js (v15) — Live overlay (dashed line + label), Import/Export CSV panel, SSE, Excel-friendly CSV */
...
function renderTradeChart(series){
  ...
  // live overlay (dashed full-width + label bubble at left)
  if(window.__livePrice){
    const yy=y(window.__livePrice);
    ctx.save();
    ctx.globalAlpha=0.35;
    ctx.setLineDash([8,6]);
    ctx.lineWidth=1;
    ctx.strokeStyle='#7fc4ff'; // أزرق هادئ
    ctx.beginPath(); ctx.moveTo(0,yy); ctx.lineTo(W,yy); ctx.stroke();
    ctx.setLineDash([]);
    // label
    const label = `Live: ${nf2.format(window.__livePrice)}`;
    ctx.font='12px system-ui, sans-serif';
    const pad=6, tw=ctx.measureText(label).width, th=18;
    const bx=8, by=Math.max(4, Math.min(H-th-4, yy- th/2));
    ctx.globalAlpha=0.95;
    ctx.fillStyle='rgba(0,0,0,0.55)';
    drawRoundedRect(ctx,bx,by,tw+pad*2,th,8); ctx.fill();
    ctx.fillStyle='#e6f2ff';
    ctx.fillText(label, bx+pad, by+th-6);
    ctx.restore();
  }
}
...
// ---------- floating panel (Import/Export) ----------
function ensureFloatingPanel(){
  if(document.getElementById('gsPanel')) return;
  const panel=document.createElement('div');
  panel.id='gsPanel';
  panel.style.cssText='position:fixed;top:12px;right:12px;z-index:2147483647;display:flex;gap:8px;';
  const btnImport=document.createElement('button');
  btnImport.textContent='📥 استيراد CSV';
  btnImport.style.cssText='padding:8px 10px;border-radius:10px;background:#2b3a60;color:#e6eef9;border:1px solid #3a4f7a;';
  const btnExport=document.createElement('button');
  btnExport.textContent='💾 تنزيل CSV المدموج';
  btnExport.style.cssText='padding:8px 10px;border-radius:10px;background:#2b3a60;color:#e6eef9;border:1px solid #3a4f7a;';
  const input=document.createElement('input'); input.type='file'; input.accept='.csv,text/csv'; input.style.display='none';
  btnImport.addEventListener('click',()=>input.click());
  btnExport.addEventListener('click',downloadMergedCsv);
  input.addEventListener('change',()=>{ if(input.files && input.files[0]) onCsvFilePicked(input.files[0]); });
  panel.appendChild(btnImport); panel.appendChild(btnExport); document.body.appendChild(panel); document.body.appendChild(input);
}
function onCsvFilePicked(file){
  ...
  reader.onload=()=>{
    try{
      ...
      __cache={ tf: currentTF, base: (currentTF===5? rows : aggregateOHLC(rows, currentTF)) };
      buildAndRender(__cache.base);
      setConnStatus(__evtSrc? 'SSE متصل' : 'جاهز','ok');
    }catch(e){ console.error(e); alert('تعذّر قراءة CSV'); }
  };
