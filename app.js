...
(function(){
  function onCsvFilePicked(file){
    ...
    reader.onload = ()=>{
      try{
        ...
        if(!rows || !rows.length){ alert('CSV فارغ أو غير مقروء'); return; }
        const tf = currentTF;
        __cache = { tf, base: (tf===5? rows : aggregateOHLC(rows, tf)) };
  ...
  function ensureImportBtn(){
    if(document.getElementById('importCsvBtn')) return;
    const btn = document.createElement('button');
    btn.id='importCsvBtn';
    btn.textContent='📥 استيراد CSV';
    btn.style.cssText='position:fixed;bottom:14px;left:180px;z-index:9999;padding:10px 12px;border-radius:10px;background:#2b3a60;color:#e6eef9;border:1px solid #3a4f7a;';
  ...
  document.addEventListener('DOMContentLoaded', ensureImportBtn);
})();
// ==== tweak live price line (so it doesn't dominate chart) ====
(function(){
  const _render = renderTradeChart;
  renderTradeChart = function(series){
    _render(series);
    try{
      const cv=document.getElementById('tradeChart'); if(!cv||!series?.length||!window.__livePrice) return;
      const ctx=cv.getContext('2d'); const W=cv.clientWidth, H=cv.clientHeight;
      const data=series.slice(-Math.min(series.length,200));
      const min=Math.min(...data.map(d=>d.low)), max=Math.max(...data.map(d=>d.high));
      ...
      ctx.moveTo(W*0.75, yy); // ارسم خط قصير بآخر الكانفاس بدل كامل العرض
      ctx.lineTo(W, yy);
      ctx.stroke();
      ctx.restore();
    }catch{}
