/* =========================
   GoldSignals PWA — app.js (v40+)
   - Indicators toggles (RSI/MACD/EMA)
   - Auto indicators (context-aware)
   - Chart w/ price line + Live + Entry/SL/TP
   - NY pivots + local date/time
   ========================= */
...
/* Try parsing flexible date+time (YYYY-MM-DD or MM/DD/YYYY, time HH:MM[:SS]) */
function parseDateTime(dateStr, timeStr){
  ...
  if (/^\d{4}[-/]\d{2}[-/]\d{2}$/.test(d)){ // YYYY-MM-DD
    ...
    mm=+mm; dd=+dd; yy=+yy; if (yy<100) yy+=2000;
    y=yy;m=mm;day=dd;
  } else {
    // fallback: Date.parse
  ...
  // Create as UTC to be timezone-agnostic in series
  const ts = Date.UTC(y, (m-1), day, hh, mi, ss);
  return ts;
}
function toLocal(ts){
  const d = new Date(ts);
...
const ctxChart=elChart?.getContext('2d');
...
let PIVOT_MIN_DISTANCE=0.8; // dollars approx
...
async function loadCsv(path){
  ...
  // detect header
  let startIdx=0;
  if (/symbol|date|time|open|high|low|close/i.test(lines[0])) startIdx=1;
  ...
  // filter invalids
  return out.filter(r=>Number.isFinite(r.ts)).sort((a,b)=>a.ts-b.ts);
}

/* ========== Indicators ========== */
function ema(arr, period){
  const k=2/(period+1), out=Array(arr.length).fill(NaN);
  let prev;
  for (let i=0;i<arr.length;i++){
    const v=arr[i];
    if (!Number.isFinite(v)) continue;
    if (!Number.isFinite(prev)) prev=v; else prev=v*k+prev*(1-k);
    out[i]=prev;
  }
  return out;
}
function rsi(closes, p){
  ...
  let gain=0,loss=0;
  for (let i=1;i<=p;i++){ const ch=closes[i]-closes[i-1]; gain+=Math.max(0,ch); loss+=Math.max(0,-ch); }
...
function macd(closes, fast=12, slow=26, signal=9){
  ...
  const mac=closes.map((_,i)=>emaF[i]-emaS[i]), sig=ema(mac,signal);
  return {macd:mac, signal:sig, emaF, emaS};
}
...
function stochastic(series,kP=14,dP=3){
  ...
  for (let i=kP-1;i<series.length;i++){
    let s=0,c=0; for (let j=Math.max(kP-1,i-dP+1);j<=i;j++){s+=K[j];c++;} D[i]=s/c;
  }
  return {K,D};
}
...
function inNyTradingHours(ts){
  ...
  // Approx: NY 08:00–17:00 = 13:00–22:00 UTC (بدون DST دقة 90%)
  const d=new Date(ts); const t=d.getUTCHours()*60+d.getUTCMinutes();
  return t>=13*60 && t<=22*60;
}
function dailyPivotsNY(series){
  if (!series.length) return null;
  const stepMin = TF===1440?1440:5; // نعتمد 5د للملف الأساسي
  const bars = Math.max(1, Math.round(1440/stepMin));
...
function strongMTFConfirm(rows30, rows60){
  ...
  // Placeholder OK; يمكن استبدالها بقراءة إطار أعلى إذا عندك
  return true;
}
...
function resizeCanvas(){
  ...
  elChart.height = Math.max(220, Math.floor(rect.height));
}
function clearChart(){
  if(!ctxChart) return;
  ctxChart.clearRect(0,0,elChart.width, elChart.height);
  ctxChart.fillStyle='#0b1220';
  ctxChart.fillRect(0,0,elChart.width, elChart.height);
  ctxChart.font='12px system-ui';
  ctxChart.fillStyle='#9ca3af';
}
...
function updateIndicatorChips(values){
  // Hide chip if toggle off
  const show = (el, flag, textOk) => {
    if (!el) return;
    el.parentElement.style.display = flag ? 'inline-block' : 'none';
    if (flag) el.textContent = textOk;
  };
...
async function run(){
  ...
  // Live price (median: نستخدم آخر close حاليا)
  ...
  if (USE_RSI || USE_MACD || USE_EMA_TREND){
    if (!inNyTradingHours(series[i].ts)) sig='حيادي';
      ...
      if (sig!=='حيادي' && !strongMTFConfirm(null,null)) sig='حيادي';
    }
  }
  ...
  if (ctxChart){
    ...
    // draw price last N
    const N=Math.min(series.length, 180);
    const win=series.slice(series.length-N);
    const minP = Math.min(...win.map(s=>s.low));
    const maxP = Math.max(...win.map(s=>s.high));
    ...
    // live
    const yLive = priceToY(live, minP, maxP);
    drawHLine(yLive, '#ffffff');

    // Entry/SL/TP if signal
    if (sig!=='حيادي'){
      const entry=live;
      const atrNow=atrArr[i]||0; const dir = (sig==='شراء')?1:-1;
      const sl= entry - dir*SL_MULT*atrNow;
      const tp1=entry + dir*TP1_MULT*atrNow;
      const tp2=entry + dir*TP2_MULT*atrNow;
      drawHLine(priceToY(entry,minP,maxP), '#60a5fa'); // blue entry
      drawHLine(priceToY(sl,minP,maxP),    '#ef4444'); // red SL
      drawHLine(priceToY(tp1,minP,maxP),   '#10b981'); // green TP1
      drawHLine(priceToY(tp2,minP,maxP),   '#10b981'); // green TP2
    }
  }
...
[elPro, elMtf, elUseRSI, elUseMACD, elUseEMA, elUseStoch, elUseBB, elAutoInd,
 ...
 elStochK, elStochD, elBBPeriod, elBBStd, elAlertEnable, elAlertDistance].forEach(el=>{
  if(!el) return; el.addEventListener('change', run);
});
...
window.addEventListener('load', ()=>{
  ...
  run().catch(e=>{ console.error(e); alert('حصل خطأ أثناء التحليل'); });
});
