...
/* ===== [GS] Inline BB% & ATR% updater (non-invasive) ===== */
(function(){
  function nf2(v){ try{return new Intl.NumberFormat('en-US',{minimumFractionDigits:2,maximumFractionDigits:2}).format(v);}catch{return String(v);} }
  function pickLineHost(){
    const nodes = Array.from(document.querySelectorAll('div,span,p,small'));
    return nodes.find(el => /BB%\s*:/.test((el.textContent||''))) || null;
  }
  function update(){
    try{
      const host = pickLineHost();
      const K = (typeof __cache === 'object' && __cache && Array.isArray(__cache.series) && __cache.series.length) ? __cache : null;
      if (!host || !K) return;

      const s = K.series, i = s.length - 1, px = s[i]?.close;
      const atrArr = Array.isArray(K.atrArr) ? K.atrArr : [];
      const atrNow = atrArr[i];
      const atrPct = (Number.isFinite(atrNow) && Number.isFinite(px) && px > 0) ? (atrNow / px * 100) : NaN;

      let bb = K.bb;
      if (!bb || !Array.isArray(bb.up) || !Array.isArray(bb.dn)) {
        if (typeof bollinger === 'function') bb = bollinger(s, (typeof BB_PERIOD==='number'?BB_PERIOD:20), (typeof BB_STD==='number'?BB_STD:2));
        else bb = null;
      }
      const up = bb?.up?.[i], dn = bb?.dn?.[i];
      const pctB = (Number.isFinite(up) && Number.isFinite(dn) && (up - dn) !== 0 && Number.isFinite(px))
        ? ((px - dn) / (up - dn)) * 100 : NaN;

      const fmt = (v)=> Number.isFinite(v) ? nf2(v) : '—';
      host.textContent = `BB%: ${fmt(pctB)} • ATR%: ${fmt(atrPct)}`;
    }catch(e){ /* ignore */ }
  }
  window.gs_updateBBATRLine = update;
  try{
    if (typeof runAnalysis === 'function' && !runAnalysis.__gsWrapped){
      const _run = runAnalysis;
      runAnalysis = async function(){ const r = await _run.apply(this, arguments); try{ update(); }catch{} return r; };
      runAnalysis.__gsWrapped = true;
    }
  }catch{}
  try{
    if (typeof reprojectWithLive === 'function' && !reprojectWithLive.__gsWrapped){
      const _rep = reprojectWithLive;
      reprojectWithLive = function(){ const r = _rep.apply(this, arguments); try{ update(); }catch{} return r; };
      reprojectWithLive.__gsWrapped = true;
    }
  }catch{}
...
/* ===== [GS] Mini-stats host (safe, no hiding) ===== */
(function(){
  const GS_MINI_ID = 'gs-mini-stats';
  function gs_ensureMiniHost() {
    const existing = Array.from(document.querySelectorAll('div,span,p,small'))
      .find(el => /BB%\s*:/.test((el.textContent || '')));
    if (existing) {
      existing.id = GS_MINI_ID;
      existing.style.display = '';
      return existing;
    }
    let el = document.getElementById(GS_MINI_ID);
    if (!el) {
      el = document.createElement('div');
      el.id = GS_MINI_ID;
      el.style.cssText = 'margin:8px 12px;color:#9aa4ad;font-size:13px;';
      el.textContent = 'BB%: — • ATR%: —';
      const preferred =
      ...
      if (preferred && preferred.parentElement) {
        preferred.parentElement.insertBefore(el, preferred);
      } else {
  ...
  window.gs_ensureMiniHost = gs_ensureMiniHost;
})();


/* ===== [GS] Mini-stats: mount under 'حالة السوق' + hide duplicate near 'السعر الحي' ===== */
(function(){
  const GS_MINI_ID = 'gs-mini-stats';

  function findByText(txt){
    const nodes = Array.from(document.querySelectorAll('h1,h2,h3,h4,div,span,p,label,small,strong,b'));
  ...
  function gs_ensureMiniHost() {
    ...
    const marketLabel = findByText('حالة السوق');
    const existingNearMarket = marketLabel
      ? Array.from((marketLabel.parentElement || marketLabel).querySelectorAll('div,span,p,small'))
          .find(n => /BB%\s*:/.test(n.textContent||''))
      : null;
    if (existingNearMarket) {
      existingNearMarket.id = GS_MINI_ID;
      existingNearMarket.style.display='';
      return existingNearMarket;
    }

    el = document.createElement('div');
    el.id = GS_MINI_ID;
    el.style.cssText = 'margin:8px 0;color:#9aa4ad;font-size:13px;';
    el.textContent = 'BB%: — • ATR%: —';

    let mounted = false;
    if (marketLabel) mounted = insertAfter(marketLabel, el);
    if (!mounted) {
      const preferred =
        document.querySelector('.indicators') ||
        document.querySelector('.summary') ||
        document.querySelector('.panel, .card');
      if (preferred && preferred.parentElement) preferred.parentElement.insertBefore(el, preferred);
  ...
  function hideDuplicatesNearLivePrice(){
    const liveBox = findByText('السعر الحي');
    if (!liveBox) return;
    const container = liveBox.closest('*') || liveBox.parentElement;
    if (!container) return;
    container.querySelectorAll('div,span,p,small').forEach(n=>{
      if (n.id === GS_MINI_ID) return;
      const t = (n.textContent||'');
      if (/BB%\s*:/.test(t)) n.style.display='none';
    });
  }

  const oldUpdate = window.gs_updateBBATRLine;
  window.gs_updateBBATRLine = function(){
    try { if (typeof oldUpdate === 'function') oldUpdate(); } catch(e){}
    try { gs_ensureMiniHost(); hideDuplicatesNearLivePrice(); } catch(e){}
  };

  if (document.readyState === 'loading') document.addEventListener('DOMContentLoaded', window.gs_updateBBATRLine);
  else window.gs_updateBBATRLine();
})();


/* ===== [GS] Mini-stats: strict placement between 'حالة السوق' and 'Auto indicators' ===== */
(function(){
  const GS_MINI_ID = 'gs-mini-stats';

  function findByText(txt){
    const nodes = Array.from(document.querySelectorAll('h1,h2,h3,h4,div,span,p,label,small,strong,b,button'));
    return nodes.find(n => (n.textContent||'').replace(/\s+/g,' ').trim().includes(txt)) || null;
  }
  function ensureHost(){
    let el = document.getElementById(GS_MINI_ID);
    if (el) return el;
    el = document.createElement('div');
    el.id = GS_MINI_ID;
    el.style.cssText = 'margin:10px 0 6px;color:#9aa4ad;font-size:13px;';
    el.textContent = 'BB%: — • ATR%: —';
    document.body.appendChild(el); // temp mount, will be moved by placeBetween()
    return el;
  }
  function commonAncestor(a,b){
    ...
    x=b; while(x){ if (set.has(x)) return x; x=x.parentElement; }
    return document.body;
  }
  function placeBetween(host){
    const start = findByText('حالة السوق');
    const end   = findByText('Auto indicators');
    if (!start || !end) return false;
    const root = commonAncestor(start,end) || document.body;
    // insert host right before the end anchor inside the same root
    try{
      root.insertBefore(host, end);
      return true;
    }catch(e){ return false; }
  }
  function removeAllDuplicates(){
    document.querySelectorAll('div,span,p,small').forEach(n=>{
      if (n.id === GS_MINI_ID) return;
      const t = (n.textContent||'');
      if (/BB%\s*:/.test(t)) n.remove();
    });
  }

  // Wrap updater: update numbers (existing logic), then enforce placement and dedupe
  const prev = window.gs_updateBBATRLine;
  window.gs_updateBBATRLine = function(){
  ...
  // Initial fix + a short retry loop for late DOM
  function kick(){
    window.gs_updateBBATRLine();
    let n=0;
    const h=setInterval(()=>{ window.gs_updateBBATRLine(); if(++n>5) clearInterval(h); }, 500);
  }
...
/* === GS inline metrics guard: keep BB% / ATR% visible === */
(function(){
  const fmt = v => (v==null || Number.isNaN(v)) ? "—" : Number(v).toFixed(2);
  let last = { bb: null, atr: null, mode: null };
  ...
  function write() {
    const el = ensureHost();
    if (!el) return;
    el.textContent = `BB%: ${fmt(last.bb)} • ATR%: ${fmt(last.atr)}`;
    const chip = document.getElementById('indBB');
    if (chip) chip.textContent = fmt(last.bb);
  }

  function setFrom(detail) {
    if (!detail) return;
    last.bb  = (detail.bbPerc != null)  ? detail.bbPerc  : detail.bbPct;
    last.atr = (detail.atrPerc != null) ? detail.atrPerc : detail.atrPct;
    if (detail.mode) last.mode = detail.mode;
    write();
  }

  window.addEventListener('gs:state-metrics',  e => setFrom(e.detail), {passive:true});
  window.addEventListener('gs:market:metrics', e => setFrom(e.detail), {passive:true});
  window.addEventListener('gs:candles:updated', () => write(), {passive:true});
...
/* === GS guard: keep 'إعدادات البيانات' card always visible === */
(function(){
  function revealSettingsCard(){
    try{
      var card = document.querySelector('[data-gs="market-state-row"]');
      if (!card) return;
  ...
  if (document.readyState === 'loading') {
    document.addEventListener('DOMContentLoaded', revealSettingsCard, {once:true});
  } else {
    revealSettingsCard();
  }
  // أي إعادة رندر بالصفحة بتسبب اختفاء البطاقة؟ رجّعها فورًا
  try{
    var mo = new MutationObserver(function(){ revealSettingsCard(); });
    mo.observe(document.body || document.documentElement, {childList:true, subtree:true});
  }catch(e){}
  // إعادة فحص لاحقًا تحسّبًا لتأخير السكربتات
  setTimeout(revealSettingsCard, 400);
  setTimeout(revealSettingsCard, 1500);
})();
