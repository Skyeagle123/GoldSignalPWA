...
/* ===== Mini Stats (Auto-detect candles) ===== */
(function(){
  function hasOHLC(obj){
    if (!obj || typeof obj!=='object') return false;
    const k = Object.keys(obj).map(s=>String(s).toLowerCase());
    const S = new Set(k);
    const have = (a)=>a.some(x=>S.has(x));
    return have(['h','high']) && have(['l','low']) && have(['c','close']);
  }
  function scoreAsCandles(arr){
    try{
      if (!Array.isArray(arr) || arr.length < 20) return -1;
      const last = arr.slice(-20);
      let ok = 0;
      for (const it of last){ if (hasOHLC(it)) ok++; }
      return ok >= 14 ? ok : -1;
    }catch(e){ return -1; }
  }
  function crawlForCandles(){
    const visited = new Set();
    let best = {score:-1, ref:null};
    function consider(name,val){
      if (!val || visited.has(val)) return;
      visited.add(val);
      const sc = scoreAsCandles(val);
      if (sc > best.score) best = {score:sc, ref:val, name};
    }
    const suspects = ['candles','chartCandles','series','data','ohlc','ohlcv','kline','klines','priceData','bars','barSeries','candleSeries','_candles','__candles'];
    for (const s of suspects){
      try{
        const v = (window[s] !== undefined ? window[s] : (window.series && window.series[s]));
        if (v) consider(s,v);
      }catch(e){}
    }
      ...
      try{
        ...
        if (val && typeof val==='object' && !Array.isArray(val)){
          for (const sub of Object.keys(val)){
            try{ consider(name+'.'+sub, val[sub]); }catch(e){}
          }
        }
      }catch(e){}
    }
  ...
  function atr14(candles){
    ...
    for (let i=1;i<candles.length;i++){
      const hi=H(candles[i]), lo=L(candles[i]), cprev=C(candles[i-1]);
      if ([hi,lo,cprev].some(v=>v==null || !isFinite(v))) return null;
      trs.push(Math.max(hi-lo, Math.abs(hi-cprev), Math.abs(lo-cprev)));
    }
    if (trs.length < 14) return null;
    const last14 = trs.slice(-14);
    return last14.reduce((a,b)=>a+b,0)/14;
  }
  function bollPctB(candles, len=20, k=2){
    if (!Array.isArray(candles) || candles.length < len) return null;
    const closes = candles.map(C);
    if (closes.some(v=>v==null || !isFinite(v))) return null;
    const last = closes.slice(-len);
    const mean = last.reduce((a,b)=>a+b,0)/len;
    const variance = last.reduce((a,b)=>a + (b-mean)*(b-mean), 0)/len;
    const stdev = Math.sqrt(variance);
    const upper = mean + k*stdev, lower = mean - k*stdev;
    const denom = (upper-lower);
    if (!denom || !isFinite(denom)) return null;
    return ((closes[closes.length-1]-lower)/denom)*100;
  }
  function ensureNode(){
    let el = document.querySelector('#gs-mini-stats');
    if (el) return el;
    el = document.createElement('div');
    el.id='gs-mini-stats';
    el.style.cssText='position:relative;margin:8px 0;color:#9aa4ad;font-size:12px;';
    el.textContent='BB%: — • ATR%: —';
    const hook = document.querySelector('.live-price,[data-live-price]');
    if (hook && hook.parentElement) hook.parentElement.insertBefore(el, hook);
    else (document.querySelector('#app')||document.body).appendChild(el);
    return el;
  }
  let gsCandles = null;
  window.gs_setCandles = function(arr){ gsCandles = arr; };
  function pickCandles(){ return gsCandles || crawlForCandles() || null; }
  function update(){
    const el = ensureNode();
    const cnd = pickCandles();
    if (!cnd){ el.textContent='BB%: — • ATR%: —'; return; }
    const a = atr14(cnd);
    const b = bollPctB(cnd);
    const last = C(cnd[cnd.length-1]);
    const atrPct = (a!=null && last) ? (a/last*100) : null;
    const fmt = (v)=> (v==null || !isFinite(v)) ? '—' : Number(v).toFixed(2);
    el.textContent = `BB%: ${fmt(b)} • ATR%: ${fmt(atrPct)}`;
  }
  window.gs_updateMiniStats = update;
  window.addEventListener('load', ()=>{ update(); setInterval(update, 2000); });
})();
