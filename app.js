...
/* === ADD-ONLY: Beep on BUY/SELL signals (mobile-friendly) === */
(function(){
  // زر لتفعيل/كتم الصوت (يتطلب كبسة واحدة لفتح الصوت على iOS/Android)
  function addToggle(){
    if (document.getElementById('gsBeepToggle')) return;
    const b = document.createElement('button');
    b.id = 'gsBeepToggle';
    b.type = 'button';
    b.textContent = '🔔 تفعيل صوت الإشارات';
    b.style.cssText = 'position:fixed;bottom:12px;right:12px;z-index:2147483647;padding:8px 12px;border-radius:10px;background:#2b3a60;color:#e6eef9;border:1px solid #3a4f7a;cursor:pointer';
  ...
  // WebAudio — توليد beep سريع بدون تحميل ملفات
  ...
  function beep(freq=880, durMs=180, vol=0.2){
    ...
    o.start(now);
    // envelope قصيرة لتفادي طقطقة
    g.gain.setValueAtTime(vol, now);
    g.gain.exponentialRampToValueAtTime(0.0001, now + durMs/1000);
    o.stop(now + durMs/1000 + 0.02);
  }

  // لحن صغير مختلف بحسب BUY/SELL
  ...
  // منع تكرار التنبيه لنفس الإطار الزمني ونفس الإشارة
  const DEDUP_MS = 60*1000; // دقيقة
  ...
  // نلف دالة pushTradeSignal إذا كانت موجودة، وإلا نراقب الـDOM
  ...
  window.pushTradeSignal = function(obj={}){
    try {
      ...
      if ((sideRaw==='BUY' || sideRaw==='SELL') && shouldBeep(sideRaw, tf)) {
        if (sideRaw==='BUY') beepBuy(); else beepSell();
      }
    } catch(e){ /* no-op */ }
    if (typeof _push === 'function') return _push.apply(this, arguments);
  };

  // الالتقاط التلقائي من عناصر الواجهة إذا ما عم تنادي pushTradeSignal يدويًا
  function norm(t){ return (t||'').replace(/\s+/g,' ').trim(); }
  function scan(){
    ...
    for (const sel of sels){
      ...
      if (!t) continue;
      if (/BUY|شراء/i.test(t) && shouldBeep('BUY', getTF())) { beepBuy(); return; }
      if (/SELL|بيع/i.test(t) && shouldBeep('SELL', getTF())) { beepSell(); return; }
    }
  }
  const mo = new MutationObserver(()=> scan());
  mo.observe(document.documentElement, {subtree:true, childList:true, characterData:true});

  // تفعيل الزر وإتاحة الصوت بعد تحميل الصفحة
