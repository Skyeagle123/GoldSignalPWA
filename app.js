...
function applyFlagsForMode(mode){
  // لا نلمس خيارات NY/Pivot يدوياً إلا عبر الـ UI حتى تبقى متوافقة مع رغبتك
  if(mode === 'fast'){
    PRO_MODE = false;
    MTF_CONFIRM = false;
    window.gsEntryPolicy = '2of3';
    // تعطيل الفلاتر الإضافية عبر واجهة الإعدادات
    if (typeof elToggleNyHours !== 'undefined' && elToggleNyHours) elToggleNyHours.checked = true;   // تعطيل فلتر ساعات نيويورك
    if (typeof elTogglePivotFilter !== 'undefined' && elTogglePivotFilter) elTogglePivotFilter.checked = true; // تعطيل فلتر Pivot
  }else if(mode === 'safe'){
    PRO_MODE = true;
    MTF_CONFIRM = true;
...
// --- Init Mode Toggle (works even if DOM already loaded) ---
