/* GoldSignals app.js — SSE + Polling fallback + CSV age badge + /ohlc fallback */
...
async function fetchOhlcFromWorker(tfMin = 5, lookbackSec = 12 * 3600) {
  const base = 'https://gold-ticks.samer-mourtada.workers.dev';
  const tfSec = tfMin * 60;
  const url = `${base}/ohlc?tf=${tfSec}&lookback=${lookbackSec}`;
...
async function loadBaseSeries(tfMin){
  ...
  try{
    ...
    return tfMin===5? rows : aggregateOHLC(rows, tfMin);
  }catch(e){
    console.warn('CSV failed; fallback to /ohlc', e);
