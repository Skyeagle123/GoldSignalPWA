...
/* === ADD-ONLY: نقل أزرار CSV تحت "إعدادات البيانات" === */
(function(){
  ...
  function ensureFileInput(){
    ...
    if(!f){ f=document.createElement('input'); f.type='file'; f.accept='.csv'; f.id='hiddenCsvFile'; f.style.display='none'; document.body.appendChild(f); }
    if(!f.__wired){
  ...
  // اختر أفضل وظيفة تصدير متاحة (Excel XML ثم XLS ثم CSV)
  function pickExport(){
    return window.downloadMergedExcelXml || window.downloadMergedXls || window.downloadMergedCsv;
  }

  function mount(){
    const host = $('#csvControls'); if(!host) return;
    if(host.__mounted) return; host.__mounted = true;

    const bImport = btn('📥 استيراد CSV');
    const bExport = btn('💾 تنزيل CSV المدموج');
    ...
    // إخفاء الأزرار القديمة في الهيدر (مطابقة نص فقط)
    const nodes = Array.from(document.querySelectorAll('button,a,[role="button"]'));
    const norm = s => (s||'').replace(/\s+/g,' ').trim();
    for(const el of nodes){
      const t = norm(el.textContent);
      if(/(استيراد\s*CSV|CSV\s*استيراد)/i.test(t) || /(تنزيل\s*CSV|CSV\s*تنزيل|تحميل\s*CSV)/i.test(t)){
