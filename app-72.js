...
/* Runtime hook to enforce mode behavior */
function __gsApplyModeRuntime(series, atrArr){
  try{
    const i = series.length-1;
    const px = series[i]?.close;
    const a = Array.isArray(atrArr)? atrArr[i] : NaN;
    const metrics = { atrPct: (Number.isFinite(a)&&Number.isFinite(px)&&px>0)?(100*a/px):NaN, bbWidthPct: NaN };
  ...
  }catch(e){ console.warn('mode hook warn', e); }
}
...
async function runAnalysis(){
  ...
  __gsApplyModeRuntime(series, atrArr);
    __gsApplyModeRuntime(series, atrArr);
