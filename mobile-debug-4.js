...
// Local additions: Beep + Browser Notification
try { if (typeof beep === 'function') beep(); } catch(_) {}
try {
  if (typeof webNotify === 'function') {
    webNotify('تنبيه تجريبي', 'هذا اختبار للتنبيه المحلي');
  } else if ('Notification' in window) {
    if (Notification.permission === 'granted') new Notification('تنبيه تجريبي', { body: 'هذا اختبار للتنبيه المحلي' });
    else if (Notification.permission !== 'denied') {
      Notification.requestPermission().then(p=>{ if (p==='granted') new Notification('تنبيه تجريبي', { body: 'هذا اختبار للتنبيه المحلي' }); });
    }
  }
} catch(_) {}
