...
/* === GS: Live from WORKER only (unified) === */
(function(){
  if (window.__GS_WORKER_ONLY__) return; window.__GS_WORKER_ONLY__ = true;

  const WORKER_URL = 'https://workerjs.samer-mourtada.workers.dev/price';
  const FRESH_MS   = 8000; // لا نقبل أي tick أقدم من 8 ثواني
  ...
  async function get(){
    ...
    // مهم: بدون headers مخصّصة لتجنّب preflight
    const r = await fetch(u, { cache:'no-store' });
    const j = await r.json();
    const price = num(j.price ?? j.close);
    const ts    = num(j.ts ?? j.timeMs) || Date.now();
    return { price, ts, src: j.source || 'worker' };
  }

  function setKpi(p, ts){
    try{ const el = document.getElementById('livePrice'); if (el) el.textContent = fmt(p); }catch(_){}
    try{
      const lt = document.getElementById('liveTime');
      if (lt){ const d=new Date(ts); lt.textContent = d.toISOString().replace('T',' ').replace('Z',''); }
    }catch(_){}
  }

  async function tick(){
    try{
      const { price, ts } = await get();
      if (price == null) return;
      const age = Date.now() - ts;
      if (age > FRESH_MS) return; // تجاهَل قراءات قديمة
      ...
      // أعِد رسم الشارت إذا متاح
      try{ if (typeof paintLive === 'function') paintLive(price, ts); }catch(_){}
      try{ if (typeof reprojectWithLive === 'function') reprojectWithLive(); }catch(_){}
    }catch(_){}
  }

  // لو DOM اتغيّر، أعد عرض القيمة الحالية
  try{
    const mo = new MutationObserver(()=> {
