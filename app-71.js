...
function setManualIndicatorsEnabled
...
function applyMarketAwarePreset(metrics, strict){
  ...
  if (ms.ranging){
    set('useEMA', false); set('useMACD', false); set('useRSI', true);
    set('useBB', true);   set('useStoch', true);
  } else {
    set('useEMA', true);  set('useMACD', true); set('useRSI', true);
    set('useBB', !!strict); set('useStoch', false);
  }
  return ms;
}
(mode){
...
function __initGsModeToggle(){
  ...
  if (!wrap.__wired){
    wrap.addEventListener('click', (e)=>{
      ...
      if (GS_MODE==='fast' || GS_MODE==='safe'){ try{ applyPresetForMode(GS_MODE); }catch(_){}}
...
async function runAnalysis(){
  try{
    ...
    (function(){
      ...
      const metrics = {atrPct: atrPctVal, bbWidthPct: bbwPctVal};
      const act = computeActiveModeNow(metrics);
      if (GS_MODE==='auto') {
        // Auto: policy follows market, suggest once unless user edited; controls enabled
        if (act && act.mode){ PRO_MODE = (act.mode!=='fast'); MTF_CONFIRM = (act.mode!=='fast'); window.gsEntryPolicy = (act.mode==='fast'?'2of3':'all'); }
        if (!window.__autoUserEdited){ applyMarketAwarePreset(metrics, false); }
        updateModeUI(act.mode, act.reason);
        setManualIndicatorsEnabled(true);
      } else if (GS_MODE==='safe') {
        // Safe: conservative, market-aware every run; controls disabled
        PRO_MODE = true; MTF_CONFIRM = true; window.gsEntryPolicy = 'all';
        applyMarketAwarePreset(metrics, true);
        updateModeUI('safe', act.reason);
        setManualIndicatorsEnabled(false);
      } else {
        // Fast: fixed preset; controls enabled
        PRO_MODE = false; MTF_CONFIRM = false; window.gsEntryPolicy = '2of3';
        applyPresetForMode('fast');
        updateModeUI('fast', act.reason);
        setManualIndicatorsEnabled(true);
      }
    })();
    // ===========================================
