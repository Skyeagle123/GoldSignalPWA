/* GoldSignals app.js — SSE + polling fallback + CSV/Stooq/OHLC loaders + live-merge */
...
function downloadMergedCsv(){
  ...
  const merged = LAST_LIVE ? (function(series,live){ const ms=tf*60*1000; const b=Math.floor(live.timeMs/ms)*ms; const out=series.slice(); const last={...out[out.length-1]}; if(b===last.ts){ last.close=live.price; last.high=Math.max(last.high,live.price); last.low=Math.min(last.low,live.price); out[out.length-1]=last; } else if(b>last.ts){ out.push({ts:b,open:last.close,high:live.price,low:live.price,close:live.price}); } return out; })(base,LAST_LIVE) : base;
  let csv = 'Date,Open,High,Low,Close\n';
  for (const r of merged) { csv += `${new Date(r.ts).toISOString()},${r.open},${r.high},${r.low},${r.close}\n`; }
...
(elBtnRun||document.getElementById('runButton'))?.addEventListener('click', runAnalysis);
(elDl||document.getElementById('downloadCsv'))?.addEventListener('click', downloadMergedCsv);
async function runAnalysis(){
  if(elConn) elConn.textContent='الاتصال: تحميل...';
  try{
    const base=await loadBaseSeries(currentTF);
    if(elConn) elConn.textContent = (__evtSrc? 'الاتصال: SSE متصل':'الاتصال: جاهز');
    buildAndRender(base);
  }catch(e){
    if(elConn) elConn.textContent='الاتصال: فشل التحميل';
    console.error(e);
  }
}
function init(){
