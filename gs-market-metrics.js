...
/* Metrics boost: retry updates to avoid 'unknown' while candles still loading */
(function(){
  try{
    function tryUpdate(){ 
      try{ 
        var g = window.gs||window.GS||{}; 
        var c = g.candles||g.data||g.series||window.__gsCandles||null; 
        if (c && c.length) { 
          window.dispatchEvent(new CustomEvent('gs:candles:updated',{detail:{candles:c}})); 
        }
      }catch(e){}
    }
    document.addEventListener('DOMContentLoaded', function(){ setTimeout(tryUpdate, 300); setTimeout(tryUpdate, 1200); });
