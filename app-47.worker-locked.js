...
/* === GS: lock KPI to WORKER price (every 1s) === */
(function(){
  if (window.__GS_WORKER_LOCK__) return; window.__GS_WORKER_LOCK__ = true;
  ...
  async function fetchWorkerPrice(){
    ...
    try{
      if (ct.includes('json') || text.trim().startsWith('{')){
        ...
        const p = num(j.price ?? j.close);
        const ts = num(j.ts ?? j.timeMs ?? j.timestamp);
        return { price: p, timeMs: ts || Date.now() };
      }
    }catch(_) {}
    const m = text.match(/[+-]?\d+(?:\.\d+)?/g);
    const p = m ? num(m.pop()) : null;
    return { price: p, timeMs: Date.now() };
  }

  async function tick(){
    try{
      const { price, timeMs } = await fetchWorkerPrice();
      if (price == null) return;
      const now = Date.now();

      // اضبط المصدرين العامّين لتتماشى كل الواجهة
      window.__livePrice  = price;
      window.__liveTimeMs = timeMs || now;
      window.LAST_LIVE    = { price, timeMs: window.__liveTimeMs };

      // حدّث الكرت
      try{ const el = document.getElementById('livePrice'); if (el) el.textContent = fmt(price); }catch(_){}
      try{ const lt = document.getElementById('liveTime'); if (lt){ const d=new Date(window.__liveTimeMs); lt.textContent = d.toISOString().replace('T',' ').replace('Z',''); } }catch(_){}

      // ارسم على الشارت لو متوفر
      try{ if (typeof paintLive === 'function') paintLive(price, window.__liveTimeMs); }catch(_){}
      try{ if (typeof reprojectWithLive === 'function') reprojectWithLive(); }catch(_){}
    }catch(_){}
  }
