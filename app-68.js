...
(function(){
  var ids = ['useRSI','useMACD','useEMA','useBB','useStoch','rsiPeriod','emaFast','emaSlow','bbPeriod','bbStd','stochK','stochD'];
  ids.forEach(function(id){
    var el = document.getElementById(id);
    if (!el || el.__wiredAutoFlag) return;
    var mark = function(){ if (window.GS_MODE==='auto') window.__autoUserEdited = true; };
  ...
  try{
    ...
    (function(){
      ...
      if (GS_MODE==='auto') {
        ...
        applyAutoPreset({atrPct: atrPctVal, bbWidthPct: bbwPctVal});
        updateModeUI(act.mode, act.reason);
        setManualIndicatorsEnabled(true);
      } else {
        applyFlagsForMode(GS_MODE);
        applyPresetForMode(GS_MODE);
        updateModeUI(GS_MODE, '');
        setManualIndicatorsEnabled(GS_MODE!=='safe');
      }
    })();
