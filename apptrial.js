...
/* ===== Quick Mode (fast/safe/auto) — integrated with existing checkboxes ===== */
const GS_MODE_KEY = 'gs_mode';
let GS_MODE = localStorage.getItem(GS_MODE_KEY) || 'auto'; // 'fast' | 'safe' | 'auto'


/* === Helper: enable/disable manual indicator controls === */
function setManualIndicatorsEnabled(enabled){
function applyPolicyForMode

/* === Presets per mode (explicit) ========================================= */
function applyPresetForMode(mode){
  const set = (id,val)=>{ const el=document.getElementById(id); if(el){ el.checked=!!val; el.dispatchEvent(new Event('change',{bubbles:true})); } };
  // Fast: EMA + MACD + RSI; others off
  if (mode==='fast'){
    set('useEMA', true); set('useMACD', true); set('useRSI', true);
    set('useBB', false); set('useStoch', false);
  }
  // Safe: EMA + MACD + RSI + BB (أكثر تحفظًا)
  else if (mode==='safe'){
    set('useEMA', true); set('useMACD', true); set('useRSI', true);
    set('useBB', true);  set('useStoch', false);
  }
}
(mode){
  // يغيّر السياسة الداخلية فقط بدون لمس واجهة المستخدم
  if (mode === 'fast'){
    PRO_MODE = false;        // Precise OFF
    MTF_CONFIRM = false;     // MTF OFF
    window.gsEntryPolicy = '2of3';
  } else { // 'safe'
    PRO_MODE = true;         // Precise ON
    MTF_CONFIRM = true;      // MTF ON
    window.gsEntryPolicy = 'all';
  }
}

  var idsChecks = ['useRSI','useMACD','useEMA','useStoch','useBB'];
  var idsInputs = ['rsiPeriod','emaFast','emaSlow','stochK','stochD','bbPeriod','bbStd'];
  idsChecks.forEach(function(id){ var el=document.getElementById(id); if(el) el.disabled = !enabled; });
  idsInputs.forEach(function(id){ var el=document.getElementById(id); if(el) el.disabled = !enabled; });
}

function applyFlagsForMode(mode){
  // اضبط نفس الـcheckboxات التي يقرأها loadSettings() لضمان الترابط
  const elPrecise = document.getElementById('proMode');
  const elMTF     = document.getElementById('mtfConfirm');
  const elAuto    = document.getElementById('autoInd');
  if(mode === 'fast'){
    if (elPrecise) elPrecise.checked = false;   // Precise OFF
    if (elMTF)     elMTF.checked     = false;   // MTF OFF
    if (elAuto)    elAuto.checked    = false;   // Auto indicators OFF
    window.gsEntryPolicy = '2of3';
    setManualIndicatorsEnabled(true);
  } else if(mode === 'safe'){
    if (elPrecise) elPrecise.checked = true;    // Precise ON
    if (elMTF)     elMTF.checked     = true;    // MTF ON
    if (elAuto)    elAuto.checked    = true;    // Auto indicators ON
    window.gsEntryPolicy = 'all';
    setManualIndicatorsEnabled(false);
  }
}

function detectMarketActivityForAuto(ind){
  // ind: { atrPct, bbWidthPct }
  const atrOk = Number.isFinite(ind?.atrPct) && ind.atrPct >= 0.35;
  const bbwOk = Number.isFinite(ind?.bbWidthPct) && ind.bbWidthPct >= 0.90;
  const reasons = [];
  if (atrOk) reasons.push(`ATR% ${ind.atrPct.toFixed(2)}≥0.35`);
  if (bbwOk) reasons.push(`BBW ${ind.bbWidthPct.toFixed(2)}≥0.90`);
  return { active: (atrOk || bbwOk), reason: reasons.join(' / ') || 'quiet' };
}
function computeActiveModeNow(ind){
  if (GS_MODE !== 'auto') return { mode: GS_MODE, reason: 'manual' };
  const a = detectMarketActivityForAuto(ind);
  return { mode: a.active ? 'fast' : 'safe', reason: a.reason };
}
function updateModeUI(activeMode, reason){
function applyAutoPreset(metrics){
  // يختار حزمة المؤشرات حسب الحالة، لكن لا يقفلها — ويمكن للمستخدم تعديلها بعدين
  ...
  const set = (el,val)=>{ if(el){ el.checked = !!val; el.dispatchEvent(new Event('change', {bubbles:true})); } };
  ...
  // سياسة الدخول فقط (داخليًا)
  applyPolicyForMode(active ? 'fast' : 'safe');

  // اقترح الحزمة لمرة واحدة إذا المستخدم ما عدّل بعد
  if (window.__autoUserEdited) return;
  if (ranging){
    // Range preset: BB + Stoch + RSI
    set(useEMA,false); set(useMACD,false);
    set(useBB,true);   set(useStoch,true); set(useRSI,true);
  } else {
    // Trend/active preset: EMA + MACD + RSI
    set(useEMA,true);  set(useMACD,true);  set(useRSI,true);
    set(useBB,false);  set(useStoch,false);
  }
}

  const wrap = document.getElementById('gs-mode-toggle');
  const note = document.getElementById('gs-mode-note');
  if (wrap){
    wrap.querySelectorAll('button').forEach(b=>{
      b.classList.toggle('active', b.dataset.mode === GS_MODE);
    });
  }
  if (note){
    const label = (GS_MODE==='auto')
      ? `ذكي → فعّال حاليًا: <b>${activeMode==='fast'?'سريع':'حذر'}</b>${reason?` (<span style="opacity:.8">${reason}</span>)`:''}`
      : (activeMode==='fast'?'سريع':'حذر');
    note.innerHTML = `الوضع الحالي: <b>${GS_MODE==='auto'?'ذكي':(activeMode==='fast'?'سريع':'حذر')}</b>` + (GS_MODE==='auto' ? ` — ${label}` : '');
  }
}
function __initGsModeToggle(){
  const wrap = document.getElementById('gs-mode-toggle');
  if (!wrap) return;
  if (!wrap.__wired){
    wrap.addEventListener('click', (e)=>{
      ...
      if(!btn) return;
      GS_MODE = btn.dataset.mode;
      localStorage.setItem(GS_MODE_KEY, GS_MODE);
      updateModeUI(GS_MODE, '');
...
if (document.readyState === 'loading'){ document.addEventListener('DOMContentLoaded', __initGsModeToggle); } else { __initGsModeToggle(); }

['useRSI','useMACD','useEMA','useBB','useStoch','rsiPeriod','emaFast','emaSlow','bbPeriod','bbStd','stochK','stochD']
  .forEach(function(id){
    var el = document.getElementById(id);
    if (!el || el.__wiredAutoFlag) return;
    el.addEventListener('change', function(){ if (window.GS_MODE==='auto') window.__autoUserEdited = true; });
    el.addEventListener('input',  function(){ if (window.GS_MODE==='auto') window.__autoUserEdited = true; });
...
async function runAnalysis(){
  try{
    ...
    // ==== Quick Mode decision & application ====
    (function(){
      // Compute ATR% and BBW% for current point
      const iAuto = series.length-1;
      let atrPctVal = (function(){ const v = atrArr?.[iAuto]; const pxv = series[iAuto]?.close; return (Number.isFinite(v)&&Number.isFinite(pxv)&&pxv>0)?(100*v/pxv):NaN; })();
      let bbwPctVal = NaN;
      const bbAuto = (document.getElementById('useBB')?.checked) ? bollinger(series, parseInt(document.getElementById('bbPeriod')?.value||'20',10), parseFloat(document.getElementById('bbStd')?.value||'2')) : null;
      if (bbAuto && Number.isFinite(bbAuto?.up?.[iAuto]) && Number.isFinite(bbAuto?.dn?.[iAuto]) && Number.isFinite(bbAuto?.mid?.[iAuto]) && bbAuto.mid[iAuto]!==0){
        bbwPctVal = ((bbAuto.up[iAuto]-bbAuto.dn[iAuto])/bbAuto.mid[iAuto])*100;
      }
      const act = computeActiveModeNow({atrPct: atrPctVal, bbWidthPct: bbwPctVal});
      if (act && act.mode) {
        applyFlagsForMode(act.mode);          // يضبط الـcheckboxات
        // أعِد القراءة كي تعتمد الحسابات اللاحقة على القيم الجديدة (بدون لمس بقية الإعدادات)
        PRO_MODE = !!document.getElementById('proMode')?.checked;
        MTF_CONFIRM = !!document.getElementById('mtfConfirm')?.checked;
        updateModeUI(act.mode, act.reason);
      } else {
        updateModeUI(GS_MODE, '');
  setManualIndicatorsEnabled(GS_MODE!=='safe');
      }
...
(function(){
  ...
  if (elAuto && !elAuto.__wired_final){
    elAuto.addEventListener('change', ()=>{
      if (elAuto.checked) { setManualIndicatorsEnabled(false); } else { setManualIndicatorsEnabled(true); }
...
// === HARD-WIRE mode toggle so buttons always work (click/touch/pointer) ===
function __recomputeNow(){
  try{ if (typeof runAnalysis==='function') return runAnalysis(); }catch(_){}
  try{ if (typeof recomputeSignals==='function') return recomputeSignals(); }catch(_){}
  try{ if (typeof run==='function') return run(); }catch(_){}
  try{ if (typeof window.buildAdvice==='function') return window.buildAdvice(); }catch(_){}
}
function wireModeToggleHard(){
  var wrap = document.getElementById('gs-mode-toggle');
  if(!wrap || wrap.__wiredHard) return;
  ...
  btns.forEach(function(b){
    ...
    ['click','touchstart','pointerdown'].forEach(function(evt){
      b.addEventListener(evt, function(ev){
        ...
        if(!m) return;
        GS_MODE = m;
        try{ localStorage.setItem(GS_MODE_KEY, GS_MODE); }catch(_){}
        updateModeUI(GS_MODE, '');
        __recomputeNow();
      }, {passive:false});
    });
  });
  wrap.__wiredHard = true;
}
document.addEventListener('DOMContentLoaded', wireModeToggleHard);
setTimeout(wireModeToggleHard, 0);
