...
/* ===== [GS] Scoped Cautious Freeze (no full-page lock) =====
   Freezes ONLY indicators/chart/signals when mode is "حذر"; rest of UI remains interactive.
================================================================ */
(function(){
  const AREAS = ['.indicators', '.signals-panel', '.chart', '.chart-section'];

  function setAreaFrozen(enable){
    document.body.style.pointerEvents = 'auto'; // never block whole page
    const ov = document.getElementById('freeze-overlay'); if (ov) ov.remove();
    AREAS.forEach(sel => {
      document.querySelectorAll(sel).forEach(el => {
        el.style.pointerEvents = enable ? 'none' : 'auto';
        el.style.opacity = enable ? '0.6' : '';
      });
    });
  }
  window.gs_applyCautiousFreeze = setAreaFrozen;

  const norm = s => String(s||'').replace(/\s+/g,' ').trim();

  // Delegate clicks on mode chips/buttons
  document.addEventListener('click', (ev) => {
    const btn = ev.target.closest('button, .btn, .chip, .toggle, [role="button"]');
    if (!btn) return;
    const t = norm(btn.textContent);
    if (t === 'حذر') { setAreaFrozen(true);  return; }
    if (t === 'سريع' || /ذكي|تلقائي/.test(t)) { setAreaFrozen(false); return; }
  }, true);

  // Optional custom event: window.dispatchEvent(new CustomEvent('gs:mode',{detail:{mode:'حذر'}}))
  window.addEventListener('gs:mode', (e)=>{
    const m = String(e && e.detail && e.detail.mode || '').toLowerCase();
    if (m.includes('حذر') or m.startswith('caut')) setAreaFrozen(true);
    else setAreaFrozen(false);
  });

  // Initial inference
  function inferInitial(){
    const nodes = Array.from(document.querySelectorAll('div,span,p,small,strong,b,button'));
    const label = nodes.find(n => norm(n.textContent).includes('حالة السوق'));
    if (!label || !label.parentElement) return;
    const active = label.parentElement.querySelector('.active, .selected, .is-active') || null;
    if (active && /حذر/.test(norm(active.textContent))) setAreaFrozen(true);
  }
  if (document.readyState === 'loading') document.addEventListener('DOMContentLoaded', inferInitial);
  else inferInitial();
})();
