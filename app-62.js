...
function setManualIndicatorsEnabled(enabled){
  const idsChecks = ['useRSI','useMACD','useEMA','useStoch','useBB'];
  const idsInputs = ['rsiPeriod','emaFast','emaSlow','stochK','stochD','bbPeriod','bbStd'];
  idsChecks.forEach(id=>{ const el=document.getElementById(id); if(el) el.disabled = !enabled; });
  idsInputs.forEach(id=>{ const el=document.getElementById(id); if (el) el.disabled = !enabled; });
}
...
function wireModeToggleHard(){
  ...
  btns.forEach(function(b){
    ...
    var handler = function(ev){
      ...
      updateModeUI(GS_MODE, '');
      try{ if (typeof __recomputeNow==='function') __recomputeNow(); else if (typeof runAnalysis==='function') runAnalysis(); }catch(_){}
    };
...
// run once and observe DOM in case settings reload re-renders the block
try{ wireModeToggleHard(); }catch(_){}
try{
