...
/* === ADD-ONLY: Hybrid Termux/Worker routing + fetch interceptor === */
/* يفضّل localhost تلقائياً إن وُجد؛ وإلا يستخدم الـ Worker بدون ما تغيّر أي سطر من كودك */

(function(){
  try {
    if (window.__hybridSetupDone) return;
    window.__hybridSetupDone = true;

    // 1) المرشّح الذكي: يجرّب المحلي بسرعة (400ms)، وإلا بيرجع للسحابة
    window.__PRICE_CANDIDATES = window.__PRICE_CANDIDATES || [
      "http://127.0.0.1:8787/price",
      "https://gold-ticks.samer-mourtada.workers.dev/price"
    ];
    window.__ALERT_CANDIDATES = window.__ALERT_CANDIDATES || [
      "http://127.0.0.1:8787/alert",
      "https://workerjs.samer-mourtada.workers.dev/alert"
    ];

    async function pickFast(urls){
      for (const u of urls){
        try{
          const ac = new AbortController();
          const to = setTimeout(()=>ac.abort(), 400);
          const r = await fetch(u + (u.includes("?")?"&":"?") + "t=" + Date.now(), { cache:"no-store", signal: ac.signal });
          clearTimeout(to);
          if (r.ok) return u;
        }catch(e){}
      }
    ...
    // 2) قرّر نقاط النهاية المفضّلة وخزّنها
    (async () => {
      const price = await pickFast(window.__PRICE_CANDIDATES);
      const alert = await pickFast(window.__ALERT_CANDIDATES);
      window.__ENDPOINTS = { price, alert };
      console.log("[hybrid] endpoints:", window.__ENDPOINTS);
    })();

    // 3) اعترض fetch: لو الطلب رايح لسعر/تنبيهنا، أعد التوجيه إلى الأفضل
    ...
    window.fetch = async function(input, init){
      try{
        ...
        // أضف no-store + كسر كاش
        init2.cache = "no-store";
        const injectTs = (u) => u + (u.includes("?") ? "&" : "?") + "t=" + Date.now();

        const eps = window.__ENDPOINTS || {};
        if (url.includes("gold-ticks.samer-mourtada.workers.dev/price") || url === "http://127.0.0.1:8787/price") {
          if (eps.price) url = eps.price;
          url = injectTs(url);
        } else if (url.includes("workerjs.samer-mourtada.workers.dev/alert") || url === "http://127.0.0.1:8787/alert") {
          if (eps.alert) url = eps.alert;
          url = injectTs(url);
        }

        const resp = await _fetch(url || input, init2);

        // تحديث بصمة الزمن للسعر الحي بدون التأثير على دفق برنامجك
        try {
          const isPrice = (url && url.indexOf("/price") !== -1);
          if (isPrice && resp && resp.ok && resp.clone) {
            resp.clone().json().then(function(j){
              if (j && (j.ts || j.time || j.isoTime)) {
                window.__liveTimeMs = j.ts || Date.parse(j.isoTime || "") || Date.now();
              } else {
        ...
        return _fetch(input, init); // في حال صار شي، ارجع للسلوك الأصلي
      }
    };
  } catch(e){}
})();
