...
/* ==== PATCH: unify live price across card, bubble, and chart (no refresh needed) ==== */
(function(){
  ...
  function updateLiveOverlay(){
    try{
      var p = (typeof window.__livePrice === 'number' && isFinite(window.__livePrice)) ? window.__livePrice : null;
      if (p == null) return;

      // 1) update optional live bubble label if present
      var bubble = document.querySelector('[data-role="live-bubble"]');
      if (bubble) bubble.textContent = 'Live: ' + fmt2(p);

      // 2) if a dedicated live line exists (some charts), keep price in sync
      var line = window.__liveLine;
  ...
  // expose a unified setter that any live source can call
  window.__onLivePrice = function(price){
    try{
      var p = Number(price);
      if (!isFinite(p)) return;
      window.__livePrice = p;
      window.__liveTimeMs = Date.now();
      // let existing app handlers do their work
      try{ if (typeof paintLive === 'function') paintLive(p, window.__liveTimeMs); }catch(_){}
      try{ if (typeof reprojectWithLive === 'function') reprojectWithLive(); }catch(_){}
      updateLiveOverlay();
    }catch(_){}
  };

  // keep UI from drifting even if no new tick for a bit
  clearInterval(window.__liveOverlayTimer);
  window.__liveOverlayTimer = setInterval(updateLiveOverlay, 1000);

  // refresh overlay when tab becomes visible
  document.addEventListener('visibilitychange', function(){
    if (!document.hidden) updateLiveOverlay();
  });
...
/* ==== PATCH v2: also push live price into chart series + update any 'Live:' badges ==== */
(function(){
  function isFn(f){ return typeof f === 'function'; }
  function nowSec(){ return Math.floor(Date.now()/1000); }
  function pushToSeries(p){
    var cand = [
      window.__priceSeries, window.__series, window.__baseSeries, window.__ohlcSeries,
      window.priceSeries, window.series, window.mainSeries
    ];
    for (var i=0;i<cand.length;i++){
      var s = cand[i];
      if (s && (isFn(s.update) || isFn(s.setData))) {
        try {
          if (isFn(s.update)) { s.update({ time: nowSec(), value: p }); return true; }
          if (isFn(s.setData) && Array.isArray(window.__lastSeriesForChart)) {
            var arr = window.__lastSeriesForChart.slice();
            arr.push({ time: nowSec(), value: p });
            s.setData(arr);
            return true;
          }
        } catch(_){}
  ...
  function updateBadges(p){
    try{
      // explicit data-role
      var bubble = document.querySelector('[data-role="live-bubble"]');
      if (bubble) bubble.textContent = 'Live: ' + p.toFixed(2);
      // generic: any node whose text starts with 'Live:'
      var nodes = document.querySelectorAll('div,span');
      for (var n of nodes){
        var t = (n.textContent||'').trim();
        if (/^Live:\s*[\d\.,]+/.test(t)) { n.textContent = 'Live: ' + p.toFixed(2); break; }
      }
    }catch(_){}
  }
  function syncAll(){
  ...
  try{ clearInterval(window.__liveChartPatchTimer); }catch(_){}
  syncAll();
  window.__liveChartPatchTimer = setInterval(syncAll, 1200);
})();
