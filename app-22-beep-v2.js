...
/* === ADD-ONLY v2: Beep on BUY/SELL + force toggle button to always appear === */
(function(){
  // ====== WebAudio basics ======
  ...
  // try immediately, on DOMContentLoaded, on load, and via retries
  (function mountToggleRobust(){
    let tries = 0, ok = ensureToggle();
    if (ok) return;
    function tryMount(){
      if (ensureToggle() || ++tries > 50) clearInterval(t);
    }
    const t = setInterval(tryMount, 120); // retry up to ~6s
  ...
  window.pushTradeSignal = function(obj={}){
    try {
      ...
      if ((sideRaw==='BUY' || sideRaw==='SELL') && shouldBeep(sideRaw, tf)) {
        // ensure audio unlocked at first user interaction; if already enabled, beep now
  ...
  function scan(){
    ...
    for (const sel of sels){
      ...
      if (!t) continue;
      const tf = getTF();
      if (/BUY|شراء/i.test(t) && shouldBeep('BUY', tf)) { if(enabled){ beepBuy(); } return; }
      if (/SELL|بيع/i.test(t) && shouldBeep('SELL', tf)) { if(enabled){ beepSell(); } return; }
    }
  }
