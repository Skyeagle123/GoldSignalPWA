...
/* === GoldSignals SAFE PATCH (append-only) ===
   - Keeps workerjs first, then gold-ticks as fallback without touching your original logic.
   - Wraps any existing fetchLivePrice() if present.
   - Provides window.GS_fetchLivePriceSafe() you can call anywhere.
*/
(function(){
  const GS_SOURCES = [
    'https://workerjs.samer-mourtada.workers.dev/price',
  ...
  async function GS_getOne(u, timeoutMs=2500){
    const ctl = new AbortController();
    const t = setTimeout(()=>ctl.abort(), timeoutMs);
    try{
      const r = await fetch(u + (u.includes('?')?'&':'?') + 't=' + Date.now(), { cache:'no-store', mode:'cors', signal: ctl.signal });
      if(!r.ok) throw new Error('HTTP '+r.status);
      const ct=(r.headers.get('content-type')||'').toLowerCase();
      const raw = await r.text();
      const j = (ct.includes('json') || raw.trim().startsWith('{')) ? JSON.parse(raw) : null;
      if(GS_healthyJson(j)) return j;
      throw new Error('unhealthy_json');
    } finally { clearTimeout(t); }
  }
  async function GS_fetchLivePriceSafe(){
    for(const src of GS_SOURCES){
      try{
        const j = await GS_getOne(src, 2500);
        return { ok:true, used: src, price:+j.price, ts:+(j.ts||Date.now()), data: j };
      }catch(e){ /* try next */ }
    }
    return { ok:false, note:'no_healthy_source' };
  }
  // export to window
  window.GS_fetchLivePriceSafe = GS_fetchLivePriceSafe;
  // if project uses fetchLivePrice(), wrap it
  if (typeof window.fetchLivePrice === 'function'){
    const __orig = window.fetchLivePrice;
    window.fetchLivePrice = async function(){
      try{
        const r = await GS_fetchLivePriceSafe();
        if (r && r.ok) { window.__livePrice = r.price; window.__liveTimeMs = r.ts; }
        else { return await __orig.apply(this, arguments); }
      }catch(e){
        return await __orig.apply(this, arguments);
      }
    };
  }
})();
