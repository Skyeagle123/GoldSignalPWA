...
/* ====== COMPAT PATCH (HTML v17 toggles) ====== */
// New manual toggles + Auto from latest HTML
const elUseRSI = document.getElementById('useRSI');
const elUseMACD = document.getElementById('useMACD');
const elUseEMA  = document.getElementById('useEMA');
const elAutoInd = document.getElementById('autoInd');

// Default flags (kept global)
...
function loadSettings(){
  ...
  if (!isAuto){
    ...
    // Simple context policy from cached series
    let atrP = 0, emaUp=false, emaDn=false, macUp=false, macDn=false;
    try{
      const base = (window.__lastSeriesForChart || window.__lastBaseSeries || (__cache && __cache.series)) || null;
      if (Array.isArray(base) && base.length > 30){
        const closes = base.map(s=>s.close);
        const _mac = macd(base, EMA_FAST, EMA_SLOW, 9);
        const i = base.length-1;
        const a = atr(base, ATR_PERIOD)[i] || 0;
        atrP = closes[i] ? a / closes[i] : 0;
        emaUp = Number.isFinite(_mac.emaF[i]) && Number.isFinite(_mac.emaS[i]) && _mac.emaF[i] > _mac.emaS[i];
        emaDn = Number.isFinite(_mac.emaF[i]) && Number.isFinite(_mac.emaS[i]) && _mac.emaF[i] < _mac.emaS[i];
        macUp = _mac.macd[i] > 0; macDn = _mac.macd[i] < 0;
      }
    }catch(e){/* ignore */}

    const trending = (emaUp && macUp) || (emaDn && macDn);
    const volatile = atrP >= 0.003; // ~0.3%

    window.USE_EMA_TREND = trending;
    window.USE_MACD = true;
    window.USE_RSI = volatile; // use RSI only when choppy
...
function paintIndicators(rsiVal,macdVal,emaFv,emaSv,stK,stD,bbMid,bbUp,bbDn){
  ...
  const hide = (el, flag) => { if (!el) return; const node = el.parentElement || el; node.style.display = flag ? 'inline-block' : 'none'; };
  hide(document.getElementById('indRSI'),  !!window.USE_RSI);
  hide(document.getElementById('indMACD'), !!window.USE_MACD);
  hide(document.getElementById('indEMAF'), !!window.USE_EMA_TREND);
  hide(document.getElementById('indEMAS'), !!window.USE_EMA_TREND);
  hide(document.getElementById('indStoch'), !!(typeof USE_STOCH==='boolean' ? USE_STOCH : true));
  hide(document.getElementById('indBB'),    !!(typeof USE_BB==='boolean' ? USE_BB : false));
}

// 4) Robust CSV loader override (try a few common paths before failing)
...
async function fetchCsv(url){
  ...
  const base = (url && url.trim()) ? url.trim() : DEFAULT_5M_CSV;
  const list = [base, './'+base, location.pathname.replace(/\\/[^/]*$/,'/')+base, 'data/'+base];
  for (const u of list){
    try { return await tryLoad(u); } catch(e){ /* keep trying */ }
  }
  // last chance: original
  if (__orig_fetchCsv){
    try { return await __orig_fetchCsv(url); } catch(e){}
  }
  throw new Error('CSV فارغ أو غير موجود. جرب وضع الملف '+base+' في نفس مجلد الصفحة. Tried: '+tried.join(' | '));
}

// 5) Re-run on toggle changes
[elUseRSI, elUseMACD, elUseEMA, elAutoInd].forEach(el=>{
  if(!el || el.__wired) return; el.addEventListener('change', ()=>{
    try{ runAnalysis(); }catch(e){ console.error(e); }
  }); el.__wired=true;
});


/* ===== MOBILE FALLBACK LAYER ===== */

/* ====== COMPAT PATCH (Mobile-Fallback v2) ====== */
(function(){
  const $ = id => document.getElementById(id);
  const badge = document.createElement('div');
  badge.id = 'fallbackBadge';
  badge.style.cssText = 'position:fixed;left:8px;bottom:8px;padding:6px 10px;border-radius:999px;background:#334155;color:#e5e7eb;font:12px system-ui;z-index:2147483647;display:none;';
  badge.textContent = 'Demo data (fallback)';
  document.addEventListener('DOMContentLoaded', ()=> document.body.appendChild(badge));

  // expose helper to original app
  window.__compat_setFallbackUsed = function(on){
    const b = document.getElementById('fallbackBadge');
    if (!b) return;
    b.style.display = on ? 'inline-block' : 'none';
  };

  // wrap parseCsv if not present
  if (typeof parseCsv!=='function'){
    window.parseCsv = function(txt){
      ...
      const hasHead = /date|time|open|high|low|close/.test(head);
      if (hasHead) s=1;
      const out=[];
      for (let i=s;i<lines.length;i++){
        const L = lines[i].split(',').map(x=>x.trim());
        if (!L.length) continue;
        let date, time, o,h,l,c;
        if (hasHead && L.length>=7){
          date=L[L.length-6]; time=L[L.length-5]; o=+L[L.length-4]; h=+L[L.length-3]; l=+L[L.length-2]; c=+L[L.length-1];
        } else if (L.length===3){
          date=L[0]; time=L[1]; c=+L[2]; o=h=l=c;
        } else if (L.length===2){
          date=L[0]; time='00:00'; c=+L[1]; o=h=l=c;
        } else continue;
        const ts = Date.parse(date+' '+time+' UTC');
...
2025-09-30,14:35,4049.80`;
  ...
  window.fetchCsv = async function(url){
    // try original first if exists
    if (__orig_fetchCsv){
      try{
        const rows = await __orig_fetchCsv(url);
        window.__compat_setFallbackUsed(false);
        return rows;
      }catch(e){ /* continue */ }
    }
    ...
    const candidates = [
      preferred,
      base+preferred,
      base+'XAUUSD_5min.csv',
      base+'data/XAUUSD_5min.csv',
      'XAUUSD_5min.csv',
      './XAUUSD_5min.csv'
    ];
    ...
    window.__compat_setFallbackUsed(true);
    return rows;
  };
