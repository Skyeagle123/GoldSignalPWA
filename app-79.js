...
function setChartMode(mode){ chartMode = (mode==='line')?'line':'candles'; try { const h=document.getElementById('chartModeHint'); if(h) h.textContent = 'الوضع الحالي: ' + (chartMode=='line'?'خط':'شموع'); } catch(e){} renderTradeChart(window.__lastSeriesForChart, window.__lastLinesForChart); }
document.addEventListener('click', (ev)=>{ const b=ev.target; if(b && b.id==='toggleChartMode'){ setChartMode(chartMode==='line'?'candles':'line'); }});
...
function renderTradeChart(series,lines){
  ...
  cancelAnimationFrame(__raf); __raf=requestAnimationFrame(()=>{
    ...
    if(chartMode==='line'){
      // draw a simple close-price line
      ctx.strokeStyle='#60a5fa'; ctx.lineWidth=1.8; ctx.beginPath();
      ctx.moveTo(xAt(0), yAt(data[0].close));
      for(let i=1;i<data.length;i++){ ctx.lineTo(xAt(i), yAt(data[i].close)); }
      ctx.stroke();
    }else{
