...
/* === Patch: emit mode + aliases with gs:market:metrics === */
(function(){
  var last = { bbPct: null, atrPct: null, mode: null };
  function classify(bbp, atrp){
    if (atrp == null || bbp == null) return "unknown";
    if (atrp < 0.25 && bbp > 35 && bbp < 65) return "range";
    return "trend";
  }
  function grabCandles(){
    var W = window, g = W.gs || W.GS || {};
    return g.candles || g.data || g.series || W.__gsCandles || null;
  }
  function computeAndEmit(){
    try{
      var W = window;
      var candles = grabCandles();
      if (!candles || !candles.length) return;
      // Fallback calc (simple enough): reuse existing global if present
      var ev = new CustomEvent('gs:candles:updated',{detail:{candles:candles}});
      W.dispatchEvent(ev);
      // expect existing listeners to compute bb/atr; if not, we keep last
      // Re-emit richer event if last has values
      var bb = (W.gs && W.gs.market && (W.gs.market.bbPct || W.gs.market.bbPerc)) || last.bbPct;
      var atr = (W.gs && W.gs.market && (W.gs.market.atrPct || W.gs.market.atrPerc)) || last.atrPct;
      var mode = (W.gs && W.gs.market && W.gs.market.mode) || classify(bb, atr);
      last = { bbPct: bb, atrPct: atr, mode: mode };
      W.dispatchEvent(new CustomEvent('gs:market:metrics', {
        detail: { mode: mode, bbPct: bb, atrPct: atr, bbPerc: bb, atrPerc: atr }
      }));
      try{ (W.gs = W.gs || {}).market = Object.assign((W.gs.market||{}), { mode: mode, bbPct: bb, atrPct: atr }); }catch(e){}
    }catch(e){}
  }
  // emit on common hooks
  window.addEventListener('gs:market:metrics', function(e){
    var d=e.detail||{}; last={ bbPct: d.bbPerc??d.bbPct, atrPct: d.atrPerc??d.atrPct, mode: d.mode??last.mode };
  }, {passive:true});
  window.addEventListener('gs:candles:updated', function(){ setTimeout(computeAndEmit, 50); }, {passive:true});
  document.addEventListener('DOMContentLoaded', function(){ setTimeout(computeAndEmit, 250); setTimeout(computeAndEmit, 1200); });
  setTimeout(computeAndEmit, 1500);
})();
